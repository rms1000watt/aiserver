...
for (i = 0; i < bufLen; ++i)
	{
	if ((buf[i] == '.') || (buf[i] == 'E') || (buf[i] == 'e')) decimal = TRUE;
	}
 ...
 *ec = FObject_recint(gCP,gTP,&x,&count,sp);  /* DO WE HAVE AN INTEGER ?          */
   ...
   if (!_TObject_IsError(*ec))           /* DO WE HAVE A FRACTION ?          */
    {
     ...
     if ((*sp == 'e') || (*sp == 'E'))  /* SCIENTIFIC NOTATION ?            */
       ...
       *ec = FObject_recint(gCP,gTP,&y,&n,sp);   /*DO WE HAVE AN EXPONENT ?          */
...
if (strlen((char*)inputText) >= MAXTVALTEXTLEN)
    {
