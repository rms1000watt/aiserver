...
AWebPage::AWebPage(AAisMgr* ipAisMgr, long iSessionId, long iMsecToWait)
	: QHttp(ipAisMgr), cpAisMgr(ipAisMgr), cGetId(-1), cReqAborted(false), cSessionId(iSessionId), cMsecToWait(iMsecToWait)
{
...
void AWebPage::onResponseHeaderReceived(const QHttpResponseHeader& irHeader)
{
	...
	if ((irHeader.statusCode() < 200) || (irHeader.statusCode() >= 400))
	{
		cancelReadHtmlPage();
	}

	//if (cTrace) qDebug("irHeader.statusCode() = %ld", irHeader.statusCode());
}
