...
;; *******************************************************************
;;  summary:  Alice chatterbot Lambda.
;;            Acts as a semi-intelligent user interface Lambda which
...
;;              Alice:         file cabinet containing this Lambda source code.
...
;;              xmlBase:       XML database manager Lambda.
;; *******************************************************************
   pvars:(;; Common Script variables
          ...
          checkin                           ;; Check in the specified AIML vocabulary Lambda to the AliceRules file cabinet.        
          checkout                          ;; Check out the specified AIML vocabulary Lambda from the alice file cabinet.        
          ...
          takeout			 				;; Check out the specified AIML vocabulary Lambda from the alice file cabinet.        
          ;; Private internal child Lambdas
          ...
          scriptLambda                       ;; Create a script template Lambda for use with lisp tags.
   ...
   ;; **************************************************************** 
   ;; ****************************************************************
   ;; Define Alice client child Lambdas
   ;; Note: These child Lambdas are available for invocation from Alice
   ...
   (defun drop(reminderString)
      vars:(n N corePatterns result firstWord aimlLambda (aimlSource "...Source Unavailable..."))
      ;; Convert the reminder string to a vector of feature based tokens.
      (if (isString reminderString) (setq reminderString (phraseToTokens (upcase reminderString))))
      (setq N (length reminderString))
      (if (= N 0) (return true))
      ...
      (setq aimlLambda (eval "(lambda() pvars:(myTrainingMemory) myTrainingMemory)"))
      (setq aimlLambda.myTrainingMemory corePatterns)
      (setq aimlLambda.Sc aimlSource)
      (browseLib.checkin AliceFacts: (append "AliceFacts:" firstWord) aimlLambda)
      true) ; end drop        
   ...
   (defun queryHtml(queryString ...)
      ...
      (setq htmlPage (mid (browseLib.checkout "Templates" "AnswerDetails") 9 10000))
   ...
   (defun reset()
      ...
      (setq myMemory (new Structure: chatbot: #{
                                                arch: "Lambda oriented communities"
                                                ...
                                                ndevelopers: 300 
                                                name: Alice 
                                                os: "Analytic Information Server" 
   ...
   ;; ****************************************************************
   ;; Define AIML lisp tag child Lambdas
   ;; Note: These child Lambdas are available for reference from AIML
                ...
                (cond
                   ...
                   ((= tag lisp:)
                    (begin
                       ...
                       (setq answer (append answer " " (eval (compile (morph (lisp temp)) scriptLambda true))))
                    )) ; end lisp case
                   ;; Case: person
                   ((= tag person:)
                    (begin
                ...
                (cond
                   ...
                   ((= tag lisp:)
                    (begin
                       (setq temp (actionAsString record))
                       (if (= verbose true) (writeln "Evaluating lisp expression: " temp))
                       (setq temp (eval (compile (morph (lisp temp)) scriptLambda true)))
                       (cond
                         ((isVector temp) (setq answer (append answer temp)))
                         ((isStructure temp) (setq answer[(length answer)] temp))
                         (else (setq answer (append answer (phraseToTokens temp))))
   ...
   (defun aimlMatch(phrase ...)
      ...
      (_browseLibExtents.Extents.AliceRules.beginTransaction)
      (if (not (isDirectory myAimlPatterns)) (setq myAimlPatterns (new Directory:)))
      ;; ...Load the AIML response patterns for all switch tests.
      (setq N (length _browseLibExtents.Extents.AliceRules.myRepository))
      (setq switchTest (string _browseLibExtents.Extents.AliceRules.myRepository[(setq n 0) 0]))
      (while (or (= switchTest[0] #\!) (= switchTest[0] #\#)) do 
         (setq myAimlPatterns[switchWord] (checkout "AliceRules" switchWord))
         (setq switchTest (string _browseLibExtents.Extents.AliceRules.myRepository[(++ n) 0]))
         ) ; end while
      ...
      (_browseLibExtents.Extents.AliceRules.commitTransaction)
      (if (= verbose true) 
          (begin
   ...
   (defun aimlMatchWord(phrase wordIndex aimlPatterns matchList)
      ...
      ;; +++++++++++++++++
      ;; Begin the logic for the aimlMatchWord child Lambda.
         ...
         (cond
          ...
          ((= (left switchTest 2) "#{")
             (begin
               ...
               (if (and (isString switchScript) (= (left switchScript 6) "#lisp#") (= (eval (compile (morph (lisp (mid switchScript 6 1000000))) scriptLambda true)) true))
                   (goto TokenMatch:)) 
          ...
          ((= (left switchTest 6) "#lisp#")
             (begin
               (if (= (eval (compile (morph (lisp (mid switchTest 6 1000000))) scriptLambda true)) true)
                   (begin 
...
;;(writeln "here for n MATCH: " )       ;;        
   ...
   (defun chat(phrase ...)
      ...
      (setq mySearchDepth oldDepth)
      answer) ; end chat
   ;; Check in the specified AIML vocabulary Lambda to the AliceRules file cabinet.        
   (defun checkin(knowledgeBase aimlPatterns)
      vars:(n N aimlLambda currentPatterns (aimlSource "...Source Unavailable...") firstWord)
      ...
      (loop for n from 0 until N do
         ...
         (setq aimlLambda (eval "(lambda() pvars:(myTrainingMemory) myTrainingMemory)"))
         (setq aimlLambda.myTrainingMemory currentPatterns)
         (setq aimlLambda.Sc aimlSource)
         (browseLib.checkin (symbol knowledgeBase) (append knowledgeBase ":" firstWord) aimlLambda)
         ) ; end aimlPatterns loop
      true) ; end checkin
   ;; Check out the specified AIML vocabulary Lambda from the alice file cabinet.        
   (defun checkout(knowledgeBase firstWord)
      vars:(n N aimlPatterns aimlLambda aimlSource)
      ;; Load the specified AIML vocabulary (for the letter specified).
      (if (or (= knowledgeBase #void) (= knowledgeBase "")) (setq knowledgeBase "AliceRules"))
      (if (not (isDirectory myAimlPatterns)) (setq myAimlPatterns (new Directory:)))
      ...
      (setq aimlLambda (browseLib.takeout (symbol knowledgeBase) (append knowledgeBase ":" firstWord)))
      (if (not (isLambda aimlLambda)) (return #void))
      ;; Make sure the patterns are in the current Lambda and not its child Lambda.
      (if (<> aimlLambda.Pv.myTrainingMemory #void)
	      (setq aimlPatterns aimlLambda.myTrainingMemory)
	      (setq aimlPatterns (eval aimlLambda)[myTrainingMemory:])
          ) ; end if 
...
;;(writeln "aimlPatterns myLexName: " aimlPatterns)
 ...
 ;;(writeln " end extractAimlPatterns: " aimlPatterns);; LTNTemp
   ...
   ;; Create a script template Lambda for use with lisp tags.
   (defun scriptLambda() true)
   ...
   (reset)
   ;; Initialize the Lambda Smartbase universal parse tree for the AIML compiled source code.
...
;;            information is used by the Master Analyst Lambda to make
;;            guestimates about the future behavior of a stock.
;;            
;;            A Master Analyst Lambda may use many memory modules with
;;            many schemes for categorizing stock historical behavior.
;;            This Lambda is capable of storing the training memory for
...
;;  Return:   true
;; *******************************************************************
   ;; The current historical and training memory for this storage module Lambda.
   pvars:(;; The current historical and training memory for this storage module Lambda.
          (myTrainingMemory
...
;; Summary:  This Lambda implements the alice.queryScript compiler as defined
...
;;*********************************************************************
;; Compile time child Lambdas for the queryScript compiler.
;; Note: These child Lambdas support the queryScript compilation process.
;;*********************************************************************
...
;; Note: This overrides the standard error handler provided by parseLib.
;;*********************************************************************
...
;;           features for the query Lambda compiler for the Alice
;;           chatterbot Lambda.
;;
;;           This Lambda query language Acts as a semi-intelligent user 
;;           interface Lambda which is able to answer user questions, 
...
;; Parms:    none
;; return:   queryScript    The parseLib checks in a newly generated 
...
;; Summary: This Lambda implements the CDATA
