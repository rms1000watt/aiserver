...
while (indexOf < argc)
    ...
    if  ((asTag(&argv[indexOf]) == TYDATE)||(asTag(&argv[indexOf]) == TYMONEY)||(asTag(&argv[indexOf]) == TYREAL))
        {
        tmp32bits = asNumIndex(&argv[indexOf++]);
        the32bits &= tmp32bits;
        }
    else
    if (asTag(&argv[indexOf]) == TYNUM)
        {
        the32bits &= asInt(&argv[indexOf++]);
        }
    else
    if (asTag(&argv[indexOf]) == TYCHAR)
        {
        the32bits &= asChar(&argv[indexOf++]);
        }
    else
...
asInt(ret) = ((the32bits & imask) == imask) ? 1 : 0;
...
asInt(ret) = ((the32bits & imask) == 0) ? 0 : 1;
...
while (width-- > 0)
   {
   if (the32bits & imask) count = !count;
   imask <<= 1;
   }
asInt(ret) = count;
