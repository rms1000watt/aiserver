...
#define PROMPT_LEN 256  /* 20 characters for 64-bit integers + 20 characters for description */
#define TITLE_LEN 216 /* needs at least 204 characters */
CHAR                    cmdLine[CMDLINE_LEN];   /*  Debugger command line string */
CHAR                    prompt[PROMPT_LEN];     /*  Debugger prompt line string */
CHAR                    title[TITLE_LEN];       /*  Debugger window title string */
NUM						titleOffset;			/*  Debugger window title string offset */
...
if (nestedDbgSwt)
	...
	// TObject_CnvToText(LpXCONTEXT gCP,LpTHREAD gTP, LpCHAR buf, NUM maxLen, TVAL source);
	// sprintf(title, sizeof(title), "%s.In.Binding = %s", argument, source);
	// the iargument is very short, #<Lambda .......>
	// the isource is long
	// FConio_snprintf(gCP,gTP,title,"%a.In.Binding = %a",

	// Modified by Frank - 5/3/2012
	// This chunk ensures that the title string won't exceed the size of the buffer
	// and cause memory corruption
	TObject_CnvToText(gCP,gTP,title,TITLE_LEN,iargument);
	titleOffset = strlen(title);
#if defined(_GCC)
	titleOffset += snprintf(title + titleOffset,TITLE_LEN - titleOffset,".In.Binding = ");
#else
	titleOffset += sprintf_s(title + titleOffset,TITLE_LEN - titleOffset,".In.Binding = ");
#endif
	TObject_CnvToText(gCP,gTP,title + titleOffset,TITLE_LEN - titleOffset,isource);

	// The following code causes memory corruption if the isource is too long
...
#if defined(_GCC)
	snprintf(prompt,PROMPT_LEN,"Current Ip at ["INTFORMAT"] in %s",Instruction,title);
#else
    sprintf_s(prompt,PROMPT_LEN,"Current Ip at ["INTFORMAT"] in %s",Instruction,title);
#endif
	// The following code causes memory corruption if title is too long
