...
long NTService::Init(LPCTSTR szInternName,void *ServiceThread)
{

  pService = this;

  fpServiceThread = (THREAD_FC)ServiceThread;
  ServiceName = new TCHAR[lstrlen(szInternName)+1];
  lstrcpy(ServiceName,szInternName);

  SERVICE_TABLE_ENTRY stb[] =
  {
    { (TCHAR*)szInternName,(LPSERVICE_MAIN_FUNCTION) ServiceMain} ,
    { NULL, NULL }
  };
...
BOOL NTService::Install(int startType, LPCTSTR szInternName,
			LPCTSTR szDisplayName,
			LPCTSTR szFullPath, LPCTSTR szAccountName,
			LPCTSTR szPassword)
{
  BOOL ret_val=FALSE;
...
BOOL NTService::Remove(LPCTSTR szInternName)
{
...
void NTService::Start(LPCTSTR ServiceName)
{
	SC_HANDLE service;
	SC_HANDLE scm;
	SERVICE_STATUS ss;
    ...
    { 
        ...
        printf("  Current State: %d\n", (int)ss.dwCurrentState); 
        printf("  Exit Code: %d\n", (int)ss.dwWin32ExitCode); 
...
void NTService::Stop(LPCTSTR ServiceName)
{
	SC_HANDLE service;
	SC_HANDLE scm;
	SERVICE_STATUS ss;
...
void NTService::StopService()
{
  ...
  else if (fpStopHandler) // if there's no shutdown event, try the stop callback function
     (*fpStopHandler)(NULL);
...
BOOL NTService::SeekStatus(LPCTSTR szInternName, int OperationType)
{
  BOOL ret_value=FALSE;
  SC_HANDLE service, scm;
...
/* ------------------------------------------------------------------------
 -------------------------------------------------------------------------- */
BOOL NTService::IsService(LPCTSTR ServiceName)
{
  BOOL ret_value=FALSE;
  SC_HANDLE service, scm;
...
/* ------------------------------------------------------------------------
 -------------------------------------------------------------------------- */
BOOL NTService::IsRunning(LPCTSTR ServiceName)
{
  BOOL ret_value=FALSE;
  SC_HANDLE service, scm;
  SERVICE_STATUS ss;
  ...
  if ((scm = OpenSCManager(0, 0,SC_MANAGER_ENUMERATE_SERVICE)))
  {
    if ((service = OpenService(scm,ServiceName, SERVICE_QUERY_STATUS)))
    {
      if (QueryServiceStatus(service, &ss))
	  {
         ret_value = (ss.dwCurrentState == SERVICE_RUNNING);
	  }
      CloseServiceHandle(service);
    }
    CloseServiceHandle(scm);
