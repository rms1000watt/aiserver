...
{
#if 0
	// Allocate first application context block BEFORE everything else
	// Note: This code work in connection with code in SBGlue_createContext.
	size_t aFirstBlockSize = AIS_FIRSTCONTEXTPOOL;
	gAis.mpFirstBlock = (POINTER)malloc(aFirstBlockSize); 
	if (gAis.mpFirstBlock < 0) 
