...
;;
;;  
;;  Title:   Small Smarttable Test Suite 
;;
;;  Author:   Michael F. Korns
;;
;;  Project:  Small Smarttable cell access and assignment study.
...
(writeln "Smalltable test suite initialization started")
...
;;  Create the required smalltable .
(define AddressList "Address List")
(define AddressListST 
    (addView (makeSmarttable small: 
                   (makeStructure name: key: city: key: salary: formula:))))
...
(if (<> AddressListST.colVector[0].valueIndex[2] "Sam Spade") 
    (diagnostic "*FAILURE*: name: column valueIndex[2] is wrong"))
(if (<> (find AddressListST.colVector[0].valueIndex "Sam Spade") 2) 
    (diagnostic "*FAILURE*: name: column find on valueIndex is wrong"))
...
(setq myMatrix (addView (makeView AddressListST matrix: "myMatrix")))
...
(setq myHyper (addView (makeView AddressListST hyper: "myHyper")))
...
;;  Import data into the Smalltable.
(setq memFree (inspect))
(setq startTime (getTickCount 0))
(importAddressList 1000 true)
(setq endTime (getTickCount startTime))
(writeln "time required for creating a Smalltable is " endTime " Seconds" )
(writeln "space required for a Smalltable is " (- memFree (inspect)) " bytes")
...
;;  Test the import.tab procedures for the stocks.sbf Smarttable.
(setq memFree (inspect))
(setq startTime (getTickCount 0))
(setq fileID (fileOpen "stocks.sbf" 0 0))
(setq Stocks (addView (makeSmarttable small:)))
(setq Stocks.name Stocks:)  
(importTab fileID Stocks)
(fileClose fileID 1)
(setq endTime (getTickCount startTime))
(writeln "Rows imported during import.tab of Stocks.sbf is " Stocks.rowCount " rows" )
(writeln "time required for import.tab of Stocks.sbf is " endTime " Seconds" )
(writeln "space required for a Smalltable is " (- memFree (inspect)) " bytes")

;;  Test the save procedures for the Stocks Smarttable.
(setq startTime (getTickCount 0))
...
;;  Test the load procedures for the Stocks Smarttable.
(deleteView Stocks)
(setq Stocks #void)
(setq startTime (getTickCount 0))
...
(writeln "Smalltable test suite completed")
(clear)
