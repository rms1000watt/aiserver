...
The FObject_recint function recognizes integers. If an integer is recognized, it 
is returned in the specified argument as a REAL (this allows very large
integers). The FObject_recint function returns the length of the integer recognized, 
or false if no integer is recognized.

#endif

TVAL FObject_recint(LpXCONTEXT gCP, LpTHREAD gTP, LpREAL dp,LpNUM count,LpCHAR sp)
{
 REAL      sign;
 NUM       sinc;
...
{
 REAL         x;                        /* TEMPORARY CONVERSION ROUTINE     */
 StartFrame
 DeclareTVAL(ec);                       /* TEMPORARY ERROR RETURN VALUE     */
 EndFrame

 *ec = FObject_recint(gCP, gTP, &x, n, sp);   /* CONVERSION FAILED ?              */
 if (_TObject_IsError(*ec))       /* CONVERSION FAILED ?              */
    FrameExit(gCP->TObject_ERROR_INVALID);   /* YES, THEN RETURN FAILED          */
 *n = x;                                /* CONVERT REAL TO AN NUM           */
...
if (oldValue.Tag == TYCPX)
	{
	retTval->Tag = TYBOLE;
	retTval->u.Bool = (oldValue.u.Complex->itsReal == 1.0) ? 1 : 0;
	FrameExit(*retTval);
	}
else
...
if ((asTag(&oldValue) == TYREAL) || (asTag(&oldValue) == TYDATE) || (asTag(&oldValue) == TYMONEY))
    {
    if (FFloat_NUMCHECK(gCP, gTP, oldValue.u.Real))
        asBool(retTval) = (asReal(&oldValue) ? 1 : 0);
    else
        *retTval = TObject_Error(gCP, gTP, "!matherror!");
    FrameExit(*retTval);
    }
else
    FrameExit(gCP->TObject_ERROR_INVALID);
...
if (oldValue.Tag == TYCPX)
	{
	retTval->Tag = TYNUM;
	//retTval->u.Int = INTEGER(oldValue.u.Complex->itsReal);
	retTval->u.Int = (NUM)(oldValue.u.Complex->itsReal);
	FrameExit(*retTval);
	}
else
if (_TObject_TypeFlag(asTag(&oldValue)) == _TObject_TfTOBJECT)
...
if ((asTag(&oldValue) == TYREAL) || (asTag(&oldValue) == TYDATE) || (asTag(&oldValue) == TYMONEY))
    {
	if (FFloat_NUMCHECK(gCP, gTP, oldValue.u.Real))
	    retTval->u.Real = oldValue.u.Real;
    else
	    *retTval = TObject_Error(gCP,gTP,"!RealAnyCnv: matherror!");
    FrameExit(*retTval);
    }
else
if (asTag(&oldValue) == TYTEXT)
...
if ((asTag(&oldValue) == TYBOLE) || (asTag(&oldValue) == TYCHAR))
    {	
    retTval->u.Real = (REAL)oldValue.u.Char;
    FrameExit(*retTval)
    }
else
if ((asTag(&oldValue) == TYNUM) || (asTag(&oldValue) == TYPOINTER))
    {
    retTval->u.Real = (REAL)oldValue.u.Int;
    FrameExit(*retTval)
    }
else
if ((asTag(&oldValue) == TYCOMPARE) || (asTag(&oldValue) == TYTYPE))
    {
    retTval->u.Real = (REAL)oldValue.u.Short;
    FrameExit(*retTval)
    }
else
if (asTag(&oldValue) == TYVOID)
