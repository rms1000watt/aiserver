	...
	{	long aStatus;
		...
		{	// _ais|logon|user|%s|passwd|%s|context|%s|wait|%d
			...
			{	QString aLogonErr;
				...
				{	aStatus = -aUsrId;
					aUsrId = 0;
					aLogonErr = gAis.mpErrMsgs[-aUsrId];
				}
				aReqType = aIsAis ? REQTYPE(aAct) : geAmpMsg;
				retOutput(aConnectId, iRqId, aStatus, aReqType, aUsrId/*Value*/, aOut, NULL,0,cMt,aLogonErr,ipInProcClient,ipSocket);
				if (ipData != NULL)
				{	free(ipData);
					qDebug("AAppSvr::submit(RqId:%ld, Ampmsg:%s), Data dropped.", iRqId, irAmpmsg.toAscii().data());
				}
