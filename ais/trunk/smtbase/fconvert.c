...
ExitOnError(*ec);
FProcedure_NewCProcedure(gCP,gTP,&savSymbol,(LpCHAR)"uinteger",(LpFUNC)&FConvert_ToUInteger);
ExitOnError(*ec);
...
The FConvert_ToUInteger function coerces the argument into an Unsigned Integer.

Note:   Exactly one argument is expected, anything else is an error.

#endif

TVAL FConvert_ToUInteger(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
StartFrame
DeclareTVAL(ret);
DeclareTVAL(err);
...
*err = FObject_UIntAnyCnv(gCP,gTP,TYREAL, argv[0]);
ExitOnError( *err);
*ret = *err;
