...
switch (pcode.u.Pcode)
                        ...
                        if ((i < 0) || (i >= BrickFromRow(isource)->itsFieldList.u.Structure->itsMaxItemIndex))
                            ...
                            if (bindPtr->Value.Modifier > 1 && bindPtr->Value.DeclaredType != TYCHAR)
							    ...
							    switch (bindPtr->Value.DeclaredType)
								    ...
								    case TYBOLE:
									    retValue.Tag = TYBOLE;
									    retValue.u.Bool = *((LpCHAR)(fieldArrayPtr + bindPtr->Value.Offset));
									    break;
    ...
    case vmregJmpEQImmediate:
		...
		m=*(Ip++);n=*(Ip++);if ((*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))) == m) Ip = (NUM*)&pcodes[n];
		goto Fetch;
		break;
    ...
    case vmregJmpEQInteger:
		...
		n=*(Ip++);if ((*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))) == (*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))) Ip = (NUM*)&pcodes[n];
		goto Fetch;
		break;
    ...
    case vmregJmpEQNumber:
		...
		n=*(Ip++);if ((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))) == (*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))) Ip = (NUM*)&pcodes[n];
		goto Fetch;
		break;
    ...
    case vmregLoadJmpPointer:
		...
		*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = (NUM)&pcodes[*(Ip++)];
		goto Fetch;
		break;
   ...
   case vmregSetXLong:
    ...
    case vmregStringiCompare:
		...
		{	//Add code to replace stricmp which is not always included in the run-time library.
			...
			*((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))) = (NUM)((tolower(*s) - tolower(*t)) < 0) ? -1 : 1;
		}
		goto Fetch;
		break;
    ...
    case vmregTanNumber:
		...
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = sin((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))) / cos((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))); 
		goto Fetch;
		break;
