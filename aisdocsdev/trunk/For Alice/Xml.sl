...
;; *******************************************************************
    pvars:(;; Public child methods
           ...
           (buffer #void)                   ;; Incomplete document model referenced by the agents
          ...
          (myVerboseSwitch false)           ;; Set the timing tests 
           (recContentsCtr 0)
           (recElementCtr 0)
            (startTimeRecContents 0)
             (endTimeRecContents 0)
           (doctypeFlag false)
           (elementName #void) 
...
(defun dictionaryOne(inputText)
   ...
   vars: (textVector n N tempString tempStringCount next)
...
(defun dictionaryTwo(inputText)
  pvars: (myDictionaryTwo)
   vars: (textVector  nextWord keyword keywordDict valueDict m n M N temp lastWord next)
...
outputVector) ;; end defun
    ...
    (setq recContentsCtr 0)
     ...
     ;;(setq startTimeRecContents 0)
     ;;(setq endTimeRecContents 0)
  (writeln " =========  START ================= ")
(setq totalRecContents 0) (setq totalRecElement 0)
    ...
    ;; Recognize the content of the XML document.
     (setq startTimeRecContents (getTickCount 0)) 
   ...
   (writeln "Elapsed time for  RecContents  is " totalRecContents " Seconds" )
    (writeln "Elapsed time for  Recelement  is " totalRecElement " Seconds" )
    ;;(setq buffer (prettyPrint buffer work.Document 2))
    (writeln "recContentsCtr: " recContentsCtr)
(writeln "recElementCtr: " recElementCtr)
 ...
 (setq endTimeRecContents (getTickCount startTimeRecContents)) 
      (writeln "Elapsed time for Main XML is " endTimeRecContents " Seconds" ) 
...
vars:(name value attributes result)
...
(writeln "recCData: sizeof cdata: " (sizeof cdata)) ;; LTNTemp
...
(writeln "recCData: sizeof comments: " (sizeof comment)) ;; LTNTemp
  Error::
...
;; ******************************************************************************************
    ...
    (setq startTimeRecContents1 0)
(++ recContentsCtr)

    (if (= myVerboseSwitch true) (setq startTimeRecContents1 (getTickCount 0)))
     
    (setq startTimeRecContents1 (getTickCount 0))


    (setq myBlanks (append myBlanks " "))
    ;;(writeln myBlanks "after myBlanks: " )
    ;;(writeln myBlanks (++ recContentsCtr)) ;; LTNTemp    
    ...
    (if (= (setq cc start[1]) #\/)
        ...
        (begin   
		  ...
		  ;; Promote input pointer past whitespace
          ;;(if (= traceLinesOn true)(writeln "recContents: LessThan:: end of element: NO Tag Match:   "   ))
          ;;(vmregRunInHardware start:)
          ;;(setq cc start[0])
          ;;(while (and (> cc 0) (<= cc 32)) (begin (++ start) (setq cc start[0])))
          ;;(vmregRunInHardware stop:)
...
;;(if (= traceLinesOn true)(writeln "recContents: document: " document))
    ...
    (while (and (<> (setq cc stringBufp[0]) #\<) (<> cc nill)) 
             
         (if (isNegative cc) (setq cc 0))   
         (setq saveCH cc)
         ;;(writeln " saveCH for stringBuf is: " (char saveCH))
         (setq stringBuf n saveCH)
         (setq stringBuf (++ n) 0)
         (setq c1 stringBufp[1])
    ...
    (setq contentName (new String: ""))
    (resize contentName (++ len))	
	...
	(if (= whitespace 0)
	    ...
	    ;; Convert the recognized char data into a string. 
             (setq contentNamep work.IP)
             ;;(setq start work.IP) 
             (if (= traceLinesOn true)(writeln "recContents:  Default Label:  whitespace is false:  " (char contentNamep[0]))) ;; LTN                 
			 (setq work.IP 0)                      
             (setq len 0)              
             ;;(setq contentName (new Vector: Byte: 1000000))  
             (vmregRunInHardware start:)  
             (setq n1 0)           
             (while (and (<> (setq saveCH contentNamep[0]) 0) (<> saveCH #\<)) 
              ...
              ;; FOR STRING START
                    (if (= stemmingOn true)
                        (begin
                            ...
                            (if (= traceLinesOn true)(writeln "stemVector is: " stemVector))
              ...
              (if (= work.eventHandler true)
			      (begin
			       ...
			       (begin
			            (setq work.IP contentNamep) 
                        (if (= traceLinesOn true)(writeln "recContents: eventHandler is false: work.IP gets contentNamep: "  contentNamep[0] )) ;; LTN  
...
;;(writeln "PI bug" )
     (setq work.IP start)
     (if (= traceLinesOn true)(writeln "recContents: end of reContents processing: " start[0] )) ;; LTN
     (goto Fetch:)
    ...
    (if (= myVerboseSwitch true) (setq endTimeRecContents1 (getTickCount startTimeRecContents1)))
(setq endTimeRecContents1 (getTickCount startTimeRecContents1))
    (if (= myVerboseSwitch true) (writeln "Elapsed time for  RecContents [" recContentsCtr "] is " endTimeRecContents1 " Seconds" ))
    (setq totalRecContents (+ totalRecContents endTimeRecContents1))

   ;;(writeln "recContents: sizeof tempChar: " (sizeof tempChar)) ;; LTNTemp
   ;;(writeln "recContents: sizeof contentName: " (sizeof contentName)) ;; LTNTemp
...
(writeln "recCData: sizeof doctypeHeader: " (sizeof doctypeHeader)) ;; LTNTemp
...
(setq startTimeRecElement 0)
(setq endTimeRecElement 0)
 
(++ recElementCtr)
(if (= myVerboseSwitch true) (setq startTimeRecElement (getTickCount 0)))
(setq startTimeRecElement (getTickCount 0))
...
;;(writeln "recElement: sizeof tempChar: " (sizeof tempChar)) ;; LTNTemp
...
;;(writeln "Decrementing reCElement: " recElementCtr)
   ...
   (setq endTimeRecElement (getTickCount startTimeRecElement)) 
    (if (= myVerboseSwitch true) (writeln "Elapsed time for  RecElement [" recElementCtr "] is " endTimeRecElement " Seconds" ))


   (setq totalRecElement (+ totalRecElement endTimeRecElement))
...
(writeln "recProcessInstruction: sizeof pInstruction: " (sizeof pInstruction)) ;; LTNTemp
...
;; Return an XML invalid scripting instruction error. */
(writeln "recScriptInstruction: sizeof script: " (sizeof script)) ;; LTNTemp
...
(writeln "recScriptInstruction: sizeof valueName before symbol: " (sizeof valueName)) ;; LTNTemp
(setq valueName (symbol valueName))
        (setq start work.IP) ;; LTN for debugging only
...
(writeln "recScriptInstruction: sizeof valueName aftrer symbol: " (sizeof valueName)) ;; LTNTemp
...
regs:((CharPointer:start)                   ;; Temporary variable to store the starting pointer
      ...
      (Integer:cc)                         ;; Temporary input character
       ...
       (Integer:null 0)   )
vars: (  line1 tempLine1 x
         line2 t tabs tempLine
         line3 lineno notabs
         line4 errorLine errorMsg
...
;;         ;;Create a long error message
        ...
        (if (and (<> line1 "") (<> line3 ""))  (setq errorMsg (append   line1 #\newline line2 #\newline errorLine #\newline line3  )))
         ...
         (writeln errorMsg)          
         (error errorString )
(writeln " :xmlError: inputString " (sizeof inputString))
