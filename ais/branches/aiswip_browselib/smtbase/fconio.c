...
while (++indexOf < argc)
    {
    /*  Convert the next argument to text before printing. */
    if (argv[indexOf].Tag == TYCHAR)
        {
        buf[0] = argv[indexOf].u.Char;
        buf[1] = 0;
        }
    else
        {
        *ec = FConio_sprint(gCP, gTP, _FSmartbase_MAXBUFFERLEN, buf,argv[indexOf]);
        ExitOnError( *ec);
        }
        
    /*  Print the converted text which is now in the print buffer. */
    if ((asTag(&argv[indexOf]) == TYTEXT) || (asTag(&argv[indexOf]) == TYSTRING) || (asTag(&argv[indexOf]) == TYSTRINGSUBSTR))
        {
        ...
        if (size > 1)
            {
            buf[size-1] = 0;
            FConio_printf(gCP, gTP, (LpCHAR)"%s",(LpCHAR)&buf[1]);
            }
        }
    else
        {
...
*ec = FConio_Display(gCP,gTP,argc,argv);
ExitOnError(*ec);

FConio_Newline(gCP,gTP,0,argv);      
...
for (argIndex = 1; argIndex < argc; ++argIndex)
    ...
    switch (argv[argIndex].Tag)
        ...
        case TYREAL:
			if (FFloat_NUMCHECK(gCP,gTP,argv[argIndex].u.Real))
				{
        ...
        case TYMONEY:
			if (FFloat_NUMCHECK(gCP,gTP,argv[argIndex].u.Real))
				{
				tmpBuf[0] = '$';
				TObject_sprintReal(gCP,gTP,(char *)&tmpBuf[1],argv[argIndex].u.Real);
				}
			else
				{
				strcpy(tmpBuf,"!matherror!");
