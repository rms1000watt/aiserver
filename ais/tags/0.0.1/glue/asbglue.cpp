...
LBLSystemThrowError:
	...
	// as AIS may be shutting down when SBGlue_CloseContext was called.
...
------------------------------------------------------------------*/
long SBGlue_Eval(void *ipCP, long iSessionID, long aRequestID, long iUserID, long iLevel,long iEvalType, long iNoDebug,long iSetJmp,
long iJitMode, char* ipCmdString, char* ipData)
{
	char*		aAmpCmd = NULL;
	LpXCONTEXT	gCP;
	LpTHREAD	gTP;
...
NUM  SBGlue_Display(LpXCONTEXT gCP,LpTHREAD gTP, char* ipString, NUM  iNewline)
{
	...
	{
		_WriteLogTime
		SysGlue_write(gCP, gTP, gCP->logFileID, aDsplyBfr.length(), (char*)aDsplyBfr.toAscii().constData());
		if (iNewline > 0)
		{
	...
	gpSessionManager->bfrDisplayOutput(gTP->SessionID, aDsplyBfr);
	return 0;
}
...
//	aEc = FSmartbase_RegisterCProcedure(gCP,gTP,(LpCHAR)"getMachineName",(LpFUNC)&SBGlue_GetMachineName);
	...
	FSmartbase_Evals(gCP,  gTP, "(lock _globals)", false);
