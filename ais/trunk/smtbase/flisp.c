...
while( hNdx < hLen  )
    ...
    switch ((curChar = atHMChar(hSrc,hNdx++)))
        ...
        case '#':
            ...
            ((&atHMChar(hSrc,0))[hNdx] <= '9') 
            ) 
                {
                /*  We attempt to see if this is an unsigned integer constant: #18446744073709551615 */
                savNdx = hNdx - 1;
                *ret = FProcedure_recInteger(gCP,gTP,&intResult,&realResult,&len,&atHMChar(hSrc,hNdx));
				if ((ret->Tag != TYBOLE) || (ret->u.Bool != TRUE))
					...
					{
					RecognizeUnsignedIntegers:
					hNdx += len;
					tmp->u.UInt = intResult;
					tmp->Tag = TYUNUM;

					if ((&atHMChar(hSrc,0))[hNdx] == '.') goto InvalidUnsignedInteger;

					/* Create an atom from the unsigned integer and add to parse tree */
        ...
        case 13:
            ...
            if ((theProc != NIL) && (vNdx < hNdx) && (gTP->FCompile_GenerateDebugInfo == TRUE))
				...
				if (dbgSourceLines != NIL)
					...
					if ((curChar == RETURNCHR) && (tmpChar == 0)) { vNdx = hNdx; goto CleanUp; }
					else
					if ((curChar == LINEBREAK) && (tmpChar == 0)) {	vNdx = hNdx; goto CleanUp; }
					else
					if ((curChar == RETURNCHR) && (tmpChar == LINEBREAK)) { ++hNdx; vNdx = hNdx; }
					else
					if ((curChar == LINEBREAK) && (tmpChar == RETURNCHR)) { ++hNdx; vNdx = hNdx; }
					else
 						vNdx = hNdx;
					}
				else
...
if (theProc != NIL)
	...
	/*  Make sure we don't try to access a negative index */
	if (vNdx < hNdx)
		{
		FLisp_SaveSrcText(gCP, gTP, dbgSourceLines, vNdx, hNdx, hSrc); 
		sourceLineNumber = dbgSourceLines->itsMaxItemIndex;       
