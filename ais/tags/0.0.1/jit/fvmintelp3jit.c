...
#define REGSAVEEAX		if (Ir[1] != 0){*Np++ = 0x89; *Np++ = REGRPMOD[1];*(LpNUM)Np = (RpRelAddress+(Ir[1]<<BITSIZEOFTVAL));Np+=4;}

/********* iff ecx is allocated to a DRM register.  ***/
/********* Save the ecx into its memory reg loc     ***/
/* Code==> mov dword ptr Rp[Ir[2]],ecx				***/ 
#define REGSAVEECX      if (Ir[2] != 0){*Np++ = 0x89; *Np++ = REGRPMOD[2];*(LpNUM)Np = (RpRelAddress+(Ir[2]<<BITSIZEOFTVAL));Np+=4;}

/********* iff ebx is allocated to a DRM register.  ***/
/********* Save the ebx into its memory reg loc     ***/
/* Code==> mov dword ptr Rp[Ir[3]],ebx				***/ 
#define REGSAVEEBX      if (Ir[3] != 0){*Np++ = 0x89; *Np++ = REGRPMOD[3];*(LpNUM)Np = (RpRelAddress+(Ir[3]<<BITSIZEOFTVAL));Np+=4;}

/********* iff esi is allocated to a DRM register.  ***/
/********* Save the esi into its memory reg loc     ***/
/* Code==> mov dword ptr Rp[Ir[4]],esi				***/ 
#define REGSAVEESI      if (Ir[4] != 0){*Np++ = 0x89; *Np++ = REGRPMOD[4];*(LpNUM)Np = (RpRelAddress+(Ir[4]<<BITSIZEOFTVAL));Np+=4;}

/********* iff edi is allocated to a DRM register.  ***/
/********* Save the edi into its memory reg loc     ***/
/* Code==> mov dword ptr Rp[Ir[5]],edi				***/ 
#define REGSAVEEDI      if (Ir[5] != 0){*Np++ = 0x89; *Np++ = REGRPMOD[5];*(LpNUM)Np = (RpRelAddress+(Ir[5]<<BITSIZEOFTVAL));Np+=4;}

/********* iff edx is allocated to a DRM register.  ***/
/********* Save the edx into its memory reg loc     ***/
/* Code==> mov dword ptr Rp[Ir[6]],edx				***/ 
#define REGSAVEEDX      if (Ir[6] != 0){*Np++ = 0x89; *Np++ = REGRPMOD[6];*(LpNUM)Np = (RpRelAddress+(Ir[6]<<BITSIZEOFTVAL));Np+=4;}

/********* iff RMOD is allocated to a DRM register. ***/
/********* Save the reg into its memory reg loc     ***/
/* Code==> mov dword ptr Rp[RMOD],RS				***/ 
#define REGSAVEMOD(RMOD)  if (Rp[RMOD].Offset > 0){*Np++ = 0x89; *Np++ = REGRPMOD[Rp[RMOD].Offset];*(LpNUM)Np = (RpRelAddress+(RMOD<<BITSIZEOFTVAL));Np+=4;}

/********* iff eax is allocated to a DRM register.  ***/
/********* Restore the eax from its saved reg.      ***/
/* Code==> mov eax,dword ptr Rp[Ir[1]]				***/ 
#define REGLOADEAX      if (Ir[1] != 0){*Np++ = 0x8B; *Np++ = REGRPMOD[1];*(LpNUM)Np = (RpRelAddress+(Ir[1]<<BITSIZEOFTVAL));Np+=4;}

/********* iff ecx is allocated to a DRM register.  ***/
/********* Restore the ecx from its saved reg.      ***/
/* Code==> mov ecx,dword ptr Rp[Ir[2]]				***/ 
#define REGLOADECX      if (Ir[2] != 0){*Np++ = 0x8B; *Np++ = REGRPMOD[2];*(LpNUM)Np = (RpRelAddress+(Ir[2]<<BITSIZEOFTVAL));Np+=4;}

/********* iff ebx is allocated to a DRM register.  ***/
/********* Restore the ebx from its saved reg.      ***/
/* Code==> mov ebx,dword ptr Rp[Ir[3]]				***/ 
#define REGLOADEBX      if (Ir[3] != 0){*Np++ = 0x8B; *Np++ = REGRPMOD[3];*(LpNUM)Np = (RpRelAddress+(Ir[3]<<BITSIZEOFTVAL));Np+=4;}

/********* iff esi is allocated to a DRM register.  ***/
/********* Restore the esi from its saved reg.      ***/
/* Code==> mov esi,dword ptr Rp[Ir[4]]				***/ 
#define REGLOADESI      if (Ir[4] != 0){*Np++ = 0x8B; *Np++ = REGRPMOD[4];*(LpNUM)Np = (RpRelAddress+(Ir[4]<<BITSIZEOFTVAL));Np+=4;}

/********* iff edi is allocated to a DRM register.  ***/
/********* Restore the edi from its saved reg.      ***/
/* Code==> mov edi,dword ptr Rp[Ir[5]]				***/ 
#define REGLOADEDI      if (Ir[5] != 0){*Np++ = 0x8B; *Np++ = REGRPMOD[5];*(LpNUM)Np = (RpRelAddress+(Ir[5]<<BITSIZEOFTVAL));Np+=4;}

/********* iff edx is allocated to a DRM register.  ***/
/********* Restore the edx from its saved reg.      ***/
/* Code==> mov edx,dword ptr Rp[Ir[6]]				***/ 
#define REGLOADEDX      if (Ir[6] != 0){*Np++ = 0x8B; *Np++ = REGRPMOD[6];*(LpNUM)Np = (RpRelAddress+(Ir[6]<<BITSIZEOFTVAL));Np+=4;}

/********* iff RMOD is allocated to a DRM register. ***/
/********* Restore the reg from its saved reg.      ***/
/* Code==> mov RS,dword ptr Rp[RMOD]				***/ 
#define REGLOADMOD(RMOD) if (Rp[RMOD].Offset > 0){*Np++ = 0x8B; *Np++ = REGRPMOD[Rp[RMOD].Offset];*(LpNUM)Np = (RpRelAddress+(RMOD<<BITSIZEOFTVAL));Np+=4;}
...
#define MOV_IREG_VREG(IREG,VREG) {*Np++ = 0x8B; *Np++ = REGRPMOD[IREG];*(LpNUM)Np = (RpRelAddress+(VREG<<BITSIZEOFTVAL));Np+=4;}
...
#define LOADDRMREGISTER(reg)\
			if ((Vr[HARDWARESW] == TRUE) && (Rp[reg].Offset > 0))\
				...
				if (Rp[reg].Tag != TYREAL)\
					...
					*Np++ = 0x8B;*Np++ = REGRPMOD[Rp[reg].Offset];*(LpNUM)Np = (RpRelAddress+(reg<<BITSIZEOFTVAL));Np+=4;\
					}\
				else\
					{\
					/* fld qword ptr Rp[Nr[k]]			*/\
					*Np++ = 0xDD;*Np++ = 0x85;*(LpNUM)Np = (RpRelAddress+(reg<<BITSIZEOFTVAL));Np+=4;\
					/* fstp st(i)						*/\
...
#define LOADOFFSETREG(mode,modifier)\
			if (mode == AMREGISTER)\
				{\
				LOADDRMREGISTER((modifier >> BITSIZEOFTVAL))\
				}\
			else\
			if (mode <= MAXRGOFFST)\
				{\
...
#define LOADALLREGS\
			if (Vr[HARDWARESW] == TRUE)\
				...
				for (k = 1; k <= IREGMAX; ++k)\
					...
					if (Ir[k] != 0)\
						{\
						/* mov reg,dword ptr Rp[Ir[k]]	*/ *Np++ = 0x8B;*Np++ = REGRPMOD[k];*(LpNUM)Np = (RpRelAddress+(Ir[k]<<BITSIZEOFTVAL));Np+=4;\
						}\
					}\
				...
				for (k = NREGMAX; k >= 1; --k)\
					...
					if (Nr[k] != 0)\
						{\
						/* fld qword ptr Rp[Nr[k]]		*/ *Np++ = 0xDD;*Np++ = 0x85;*(LpNUM)Np = (RpRelAddress+(Nr[k]<<BITSIZEOFTVAL));Np+=4;\
						}\
					}\
				}
...
#define SAVEDRMREGISTER(reg)\
			if ((Vr[HARDWARESW] == TRUE) && (Rp[reg].Offset > 0))\
				...
				if (Rp[reg].Tag != TYREAL)\
					...
					*Np++ = 0x89;*Np++ = REGRPMOD[Rp[reg].Offset];*(LpNUM)Np = (RpRelAddress+(reg<<BITSIZEOFTVAL));Np+=4;\
					}\
				else\
					{\
					/* fld	st(reg)						*/\
					...
					*Np++ = 0xDD;*Np++ = 0x9D;*(LpNUM)Np = (RpRelAddress+(reg<<BITSIZEOFTVAL));Np+=4;\
					}\
				}
...
#define SAVEOFFSETREG(mode,modifier)\
			if (mode == AMREGISTER)\
				{\
				SAVEDRMREGISTER((modifier >> BITSIZEOFTVAL))\
				}\
			else\
			if (mode <= MAXRGOFFST)\
				{\
...
#define SAVEALLREGS\
			if (Vr[HARDWARESW] == TRUE)\
				...
				for (k = 1; k <= IREGMAX; ++k)\
					...
					if (Ir[k] != 0)\
						{\
						/* mov dword ptr Rp[Ir[k]],reg	*/ *Np++ = 0x89;*Np++ = REGRPMOD[k];*(LpNUM)Np = (RpRelAddress+(Ir[k]<<BITSIZEOFTVAL));Np+=4;\
						}\
					}\
				...
				for (k = 1; k <= NREGMAX; ++k)\
					...
					if (Nr[k] != 0)\
						{\
						/* fstp qword ptr Rp[Nr[k]]		*/ *Np++ = 0xDD;*Np++ = 0x9D;*(LpNUM)Np = (RpRelAddress+(Nr[k]<<BITSIZEOFTVAL));Np+=4;\
						}\
					}\
				}
			...
			switch (pcode.u.Pcode)
				...
				case VMPOP:
					...
					if (pcode.u.Am1 != AMVOID) 
						...
						/* mov    ecx,dword ptr Rp[Am1]                 */ *Np++ = 0x8B;*Np++ = 0x8D;*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4; 
					...
					if (pcode.u.Am2 != AMVOID) 
						...
						/* mov    ecx,dword ptr Rp[Am2]                 */ *Np++ = 0x8B;*Np++ = 0x8D;*(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						/*** Set Pc/Nc allignment                     ***/ Jp[++n] = (NUM)((Np-ncodes)+2); srcPtrID = pcodes[n];
						/* mov    eax,dword Ptr [edx+0x11223344]        */ *Np++ = 0x8B;*Np++ = 0x82;*(LpNUM)Np = tarPtrID;Np+=4;
						/* mov    dword Ptr [ecx+0x11223344],eax        */ *Np++ = 0x89;*Np++ = 0x81;*(LpNUM)Np = srcPtrID;Np+=4;
					...
					if (pcode.u.Am3 != AMVOID) 
						...
						/* mov    ecx,dword ptr Rp[Am3]                 */ *Np++ = 0x8B;*Np++ = 0x8D;*(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL)); Np+=4;
						/*** Set Pc/Nc allignment                     ***/ Jp[++n] = (NUM)((Np-ncodes)+2); srcPtrID = pcodes[n];
						/* mov    eax,dword Ptr [edx+0x11223344]        */ *Np++ = 0x8B;*Np++ = 0x82;*(LpNUM)Np = tarPtrID;Np+=4;
						/* mov    dword Ptr [ecx+0x11223344],eax        */ *Np++ = 0x89;*Np++ = 0x81;*(LpNUM)Np = srcPtrID;Np+=4;
				...
				case VMSELF:
					...
					/* mov    edx,dword ptr Rp[Am1]                 */ *Np++ = 0x8B;*Np++ = 0x95;*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
				...
				case VMARGFETCH:
					...
					/* mov    edx,dword ptr Rp[Am2]                 */ *Np++ = 0x8B;*Np++ = 0x95;*(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
				...
				case VMIADD:
        			...
        			/*** source = (Rp[Am2]+offset);               ***/
					/* mov    eax,dword ptr Rp[Am2]                 */ *Np++ = 0x8B;*Np++ = 0x85;*(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
					/*** Set Pc/Nc allignment                     ***/ Jp[++n] = (NUM)((Np-ncodes)+1);
					/* add    eax,0x11223344                        */ *Np++ = 0x05;*(LpNUM)Np = pcodes[n]; Np+=4;
        			/* target = (Rp[Am3]+offset);                 ***/
					/* mov    edx,dword ptr Rp[Am3]                 */ *Np++ = 0x8B;*Np++ = 0x95;*(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL)); Np+=4;
					/*** Set Pc/Nc allignment                     ***/ Jp[++n] = (NUM)((Np-ncodes)+2);
				...
				case VMCADD:
					...
					/* mov edx,Rp[Am2]                 */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
					/*** Set Pc/Nc allignment        ***/ Jp[++n] = (NUM)((Np-ncodes)+2);  
				...
				case vmnatLoadCharacter:
					outStream(JITOpWrd,FALSE)  // FALSE if compiling this instruction instream.
					/*** Set Pc/Nc allignment      ***/ Jp[n]   = (NUM)(Np-ncodes);        
					/* mov ecx,Rp[Am1]               */ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
				...
				case vmnatLoadObject:
					...
					/*** Set Pc/Nc allignment      ***/ Jp[++n] = (NUM)((Np-ncodes)+2); 
					/* mov eax,[ecx+0x11223344]      */ *Np++ = 0x8B; *Np++ = 0x81; *(LpNUM)Np = pcodes[n]; Np+=4;
					/* mov edx,Rp[2]                 */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
					/*** Set Pc/Nc allignment      ***/ Jp[++n] = (NUM)((Np-ncodes)+2);  
					/* mov [edx+0x11223344],eax      */ *Np++ = 0x89; *Np++ = 0x82; *(LpNUM)Np = pcodes[n];Np+=4;
				...
				case VMMOVE:
					...
					SAVEALLREGS
					/* mov    ecx,dword ptr Rp[source]              */ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
					/* mov    edx,dword ptr Rp[target]              */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
					/* mov    eax,dword Ptr [ecx+0x11223344]        */ *Np++ = 0x8B; *Np++ = 0x81; *(LpNUM)Np = srcPtrID;Np+=4;
					/* mov    dword Ptr [edx+0x11223344],eax        */ *Np++ = 0x89; *Np++ = 0x82; *(LpNUM)Np = tarPtrID;Np+=4;
					/* mov    eax,dword Ptr [ecx+4+0x11223344]      */ *Np++ = 0x8B; *Np++ = 0x81; *(LpNUM)Np = (srcPtrID+4);Np+=4;
					/* mov    dword Ptr [edx+4+0x11223344],eax      */ *Np++ = 0x89; *Np++ = 0x82; *(LpNUM)Np = (tarPtrID+4);Np+=4;
				...
				case VMSETVECTOR:
					...
					/*** Set Pc/Nc allignment                     ***/ Jp[++n] = (NUM)(Np-ncodes); srcPtrID = pcodes[n];
					/*** Set Pc/Nc allignment                     ***/ Jp[++n] = (NUM)(Np-ncodes); tarPtrID = pcodes[n];
					/* mov    edx,dword ptr Rp[index]				*/ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
					/* mov    edx,dword Ptr [edx+0x11223344]        */ *Np++ = 0x8B; *Np++ = 0x92; *(LpNUM)Np = argPtrID;Np+=4;
					/* shl    edx,4									*/ *Np++ = 0xC1; *Np++ = 0xE2; *Np++ = 0x04; 
					/* mov    ecx,dword ptr Rp[source]              */ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
					/*** Load address for &target[index]	      ***/ 
					/* mov    eax,dword ptr Rp[target]              */ *Np++ = 0x8B; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
				...
				case VMREFVECTOR:
					...
					/* mov    edx,dword Ptr [edx+0x11223344]        */ *Np++ = 0x8B; *Np++ = 0x92; *(LpNUM)Np = argPtrID;Np+=4;
					/* shl    edx,4									*/ *Np++ = 0xC1; *Np++ = 0xE2; *Np++ = 0x04; 
					/* mov    ecx,dword ptr Rp[target]              */ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
					/*** Load address for &source[index]	      ***/ 
					/* mov    eax,dword ptr Rp[source]              */ *Np++ = 0x8B; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
				...
				case vmnatJmpLTInteger:
					...
					/*** Set Pc/Nc allignment ***/ Jp[n]   = (NUM)(Np-ncodes);        
					/* mov ecx,Rp[source]       */ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
					/*** Set Pc/Nc allignment ***/ Jp[++n] = (NUM)((Np-ncodes)+2); 
					/* mov ecx,[ecx+0x11223344] */ *Np++ = 0x8B; *Np++ = 0x89; *(LpNUM)Np = pcodes[n]; Np+=4;
					/* mov edx,Rp[target]       */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
					/*** Set Pc/Nc allignment ***/ Jp[++n] = (NUM)((Np-ncodes)+2);  
			...
			switch (pcode.u.Pcode)
				...
				case vmregJump:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/* jmp dword ptr Rp[target]      */ *Np++ = 0xFF; *Np++ = 0xA5; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));	Np+=4;
						}
					else
						{
						if (Ir[Rp[modifier[0]].Offset] != 0){*Np++ = 0x89; *Np++ = REGRPMOD[Rp[modifier[0]].Offset];*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;}
						/* jmp dword ptr Rp[target]      */ *Np++ = 0xFF; *Np++ = 0xA5; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));	Np+=4;
						}
					break;
				...
				case vmregAddImmediate:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* add Rp[target],immediate          */ *Np++ = imod[0]; *Np++ = imod[1]; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4; *(LpNUM)Np = pcodes[n]; Np+=4;
						}
					else
				...
				case vmregIncPointer:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* add Rp[Am3],immediate            */ *Np++ = 0x81; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL)); Np+=4; *(LpNUM)Np = k; Np+=4;
						}
					else
				...
				case vmregAddInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/*** No hardware registers		  ***/
						/* mov  edx,Rp[Am1]                 */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
						/* add  Rp[Am2],edx                 */ *Np++ = imod[0]; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					...
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset >= 1))
						...
						/*** source in memory		 	  ***/
						/*** target in i register	      ***/
						/* add  Reg[Am2],Rp[Am1]            */ *Np++ = imod[1]; *Np++ = REGRPMOD[Rp[modifier[1]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset >= 1) && (Rp[modifier[1]].Offset == 0))
						...
						/*** source in  i			 	  ***/
						/*** target in memory		      ***/
						/* add  Rp[Am2],Reg[Am1]            */ *Np++ = imod[0]; *Np++ = REGRPMOD[Rp[modifier[0]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					break;
				...
				case vmregSubPointer:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* sub  Rp[Am3],edx                 */ *Np++ = 0x29; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					...
					if ((Rp[modifier[1]].Offset == 0) && (Rp[modifier[2]].Offset >= 1))
						...
						if (k != 0)
							...
							{
							/* sub  Reg[Am3],Rp[Am2]        */ *Np++ = 0x2B; *Np++ = REGRPMOD[Rp[modifier[2]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
							}
						}
					else
					if ((Rp[modifier[1]].Offset >= 1) && (Rp[modifier[2]].Offset == 0))
						...
						if (k != 0)
							...
							/* shl  edx,imm8                */ *Np++ = 0xC1; *Np++ = 0xE2; *Np++ = (char)k;
							/* sub Rp[Am3],edx              */ *Np++ = 0x29; *Np++ = REGRPMOD[6]; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
							}
						else
							{
							/* sub  Rp[Am3],Reg[Am2]        */ *Np++ = 0x29; *Np++ = REGRPMOD[Rp[modifier[1]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
							}
						}
					break;
				...
				case vmregAddPointer:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* add  Rp[Am3],edx             */ *Np++ = 0x01; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[1]].Offset == 0) && (Rp[modifier[2]].Offset == 0))
						{
					...
					if ((Rp[modifier[1]].Offset == 0) && (Rp[modifier[2]].Offset >= 1))
						...
						if (k != 0)
							...
							{
							/* add  Reg[Am3],Rp[Am2]        */ *Np++ = 0x03; *Np++ = REGRPMOD[Rp[modifier[2]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
							}
						}
					else
					if ((Rp[modifier[1]].Offset >= 1) && (Rp[modifier[2]].Offset == 0))
						...
						if (k != 0)
							...
							/* shl  edx,imm8                */ *Np++ = 0xC1; *Np++ = 0xE2; *Np++ = (char)k;
							/* add Rp[Am3],edx              */ *Np++ = 0x01; *Np++ = REGRPMOD[6]; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
							}
						else
							{
							/* add  Rp[Am3],Reg[Am2]        */ *Np++ = 0x01; *Np++ = REGRPMOD[Rp[modifier[1]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
							}
						}
					break;
				...
				case vmregMoveInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/*** No hardware registers		  ***/
						/* mov edx,Rp[Am1]                   */ *Np++ = 0x8B;	*Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
						/* mov Rp[Am2],edx                   */ *Np++ = 0x89;	*Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
						{
					...
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset >= 1))
						...
						/*** source in memory		 	  ***/
						/*** target in i register	      ***/
						/* mov  Reg[Am2],Rp[Am1]            */ *Np++ = 0x8B; *Np++ = REGRPMOD[Rp[modifier[1]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset >= 1) && (Rp[modifier[1]].Offset == 0))
						{
						/*** source in  i			 	  ***/
						/*** target in memory		      ***/
						/* mov  Rp[Am2],Reg[Am1]            */ *Np++ = 0x89; *Np++ = REGRPMOD[Rp[modifier[0]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					break;
				...
				case vmregMoveNumber:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/*** No hardware registers		  ***/
						/* fld   Rp[source]                 */ *Np++ = 0xDD; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* fstp  Rp[target]                 */ *Np++ = 0xDD; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
						{
				...
				case vmregRefInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* mov dword ptr Rp[Am2],edx	     */ *Np++ = 0x89; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						}
					else
						{
				...
				case vmregRefFloat:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* mov edx,dword ptr Rp[Am1]         */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* fld dword ptr [edx]               */ *Np++ = 0xD9;	*Np++ = 0x02;
						/* fstp qword ptr Rp[Am2]			 */ *Np++ = 0xDD; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						}
					else
						{
						if (Rp[modifier[0]].Offset == 0)
				...
				case vmregRefXFloat:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* add edx,dword ptr Rp[Am1]         */ *Np++ = 0x03; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* fld dword ptr [edx]               */ *Np++ = 0xD9;	*Np++ = 0x02;
						/* fstp qword ptr Rp[target]		 */ *Np++ = 0xDD; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL)); Np+=4;
						}
					else
						{
						if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
				...
				case vmregRefXInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* add edx,dword ptr Rp[Am1]         */ *Np++ = 0x03; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* mov edx,[edx]					 */ *Np++ = 0x8B;	*Np++ = 0x12;
						/* mov dword ptr Rp[Am3],edx	     */ *Np++ = 0x89; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL)); Np+=4;
						}
					else
						{
						if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
...
*Np++ = 0x90;
				...
				case vmregRefXNumber:
 				...
 				case vmregDivInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* mov  eax,dword ptr Rp[Am2]       */ *Np++ = 0x8B; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						/* cdq                              */ *Np++ = 0x99; 
						/* idiv eax,Rp[Am1]                 */ *Np++ = 0xF7;	*Np++ = 0xBD; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* mov Rp[Am2],eax                  */ *Np++ = 0x89;	*Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
						{
						if ((Rp[modifier[0]].Offset == 1) && (Rp[modifier[1]].Offset != 1))
							...
							/* mov dword ptr Rp[Ir[1]],eax	           */ *Np++ = 0x89; *Np++ = REGRPMOD[1];*(LpNUM)Np = (RpRelAddress+(Ir[1]<<BITSIZEOFTVAL));Np+=4;
							}
						else
						...
						if (Rp[modifier[1]].Offset == 0)
							{
							/*** target in memory                    ***/	
							/* mov eax,Rp[Am2]                         */ *Np++ = 0x8B; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
							}
						else
						if (Rp[modifier[1]].Offset > 1)
							{
						...
						if (Rp[modifier[0]].Offset == 0)
							{
							/* cdq                           */ *Np++ = 0x99;
							/* idiv dword ptr Rp[Am1]        */ *Np++ = 0xF7; *Np++ = 0xBD; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));  Np+=4;
							}
						else
						if ((Rp[modifier[0]].Offset == 1) && (Rp[modifier[1]].Offset != 1))
							{
						...
						if ((Rp[modifier[0]].Offset == 1) && (Rp[modifier[1]].Offset != 1))
							{
							/*** source in eax AND target NOT in eax ***/	
							/* mov eax,dword ptr Rp[Ir[1]]	     */ *Np++ = 0x8B; *Np++ = REGRPMOD[1];*(LpNUM)Np = (RpRelAddress+(Ir[1]<<BITSIZEOFTVAL));Np+=4;
							}
						else
						if ((Ir[1] != 0) && (Rp[modifier[1]].Offset != 1))
							{	
				...
				case vmregDivImmediate:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						if ((Ir[2] != 0) && (Rp[modifier[1]].Offset != 2))
							{	
							/* mov dword ptr Rp[Ir[2]],ecx	 */ *Np++ = 0x89; *Np++ = REGRPMOD[2];*(LpNUM)Np = (RpRelAddress+(Ir[2]<<BITSIZEOFTVAL));Np+=4;
							}
						if (Rp[modifier[1]].Offset == 0)
							{
							/* mov eax,Rp[Am2]                   */ *Np++ = 0x8B; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						...
						if ((Ir[1] != 0) && (Rp[modifier[1]].Offset != 1))
							{	
							/* mov reg,dword ptr Rp[Am2]	     */ *Np++ = 0x8B; *Np++ = REGRPMOD[1];*(LpNUM)Np = (RpRelAddress+(Ir[1]<<BITSIZEOFTVAL));Np+=4;
							}
						if ((Ir[2] != 0) && (Rp[modifier[1]].Offset != 2))
							{	
							/* mov reg,dword ptr Rp[Am2]	     */ *Np++ = 0x8B; *Np++ = REGRPMOD[2];*(LpNUM)Np = (RpRelAddress+(Ir[2]<<BITSIZEOFTVAL));Np+=4;
							}
						}
					break;
				...
				case vmregMulInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/*** No hardware registers		  ***/
						/* mov  edx,Rp[Am2]                 */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						/* imul edx,Rp[Am1]                 */ *Np++ = 0x0F; *Np++ = 0xAF; *Np++ = 0x95;  *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));  Np+=4;
						/* mov  Rp[Am2],edx                 */ *Np++ = 0x89; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
					...
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset >= 1))
						...
						/*** source in memory		 	  ***/
						/*** target in i register	      ***/
						/* imul  Reg[Am2],Rp[Am1]           */ *Np++ = 0x0F; *Np++ = 0xAF; *Np++ = REGRPMOD[Rp[modifier[1]].Offset]; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset >= 1) && (Rp[modifier[1]].Offset == 0))
						{
				...
				case vmregJmpLTInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/* mov edx,Rp[Am1]                   */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* cmp edx,Rp[Am2]                   */ *Np++ = 0x3B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						}
					else
						{
						if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
						...
						if ((Rp[modifier[0]].Offset >= 1) && (Rp[modifier[1]].Offset == 0))
							{
							/* cmp Reg[Am1],Rp[Am2]				 */ *Np++ = 0x3B; *Np++ = REGRPMOD[Rp[modifier[0]].Offset];*(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
							}
						else
						if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset >= 1))
							{
				...
				case vmregNumber:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/*** No hardware registers		  ***/
						/* fild  dword ptr Rp[source]       */ *Np++ = 0xDB; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* fstp  qword ptr Rp[target]       */ *Np++ = 0xDD; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
						{
				...
				case vmregInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/*** No hardware registers		  ***/
						/* fld   qword ptr Rp[source]       */ *Np++ = 0xDD; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						...
						/* fistp dword ptr Rp[target]       */ *Np++ = 0xDB; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
						{
						/*** Load the source variable to ST0  ***/
						...
						if (Rp[modifier[0]].Offset == 1)
							...
							/* fsubp   st(2),st(0)				*/ *Np++ = 0xDE; *Np++ = 0xEA;
							/* fstp    st(0)					*/ *Np++ = 0xDD; *Np++ = 0xD8;
							/* fist	 qword ptr Rp[target]       */ *Np++ = 0xDB; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
							}
						else
							{
				...
				case vmregAddNumber:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/*** No hardware registers		  ***/
						/* fld   Rp[target]                 */ *Np++ = 0xDD; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						/* fadd  Rp[source]                 */ *Np++ = 0xDC; *Np++ = imod[0]; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
						/* fstp  Rp[target]                 */ *Np++ = 0xDD; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
					...
					if ((Rp[modifier[0]].Offset >= 1) && (Rp[modifier[1]].Offset == 0))
						...
						/*** source in STi			 	  ***/
						/*** target in memory		      ***/
						/* fld   st(target)                 */ *Np++ = 0xDD; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						if (Rp[modifier[1]].Offset == 0)
							...
							if (Rp[modifier[0]].Offset >= 1)
								...
								if (Ir[5] != 0) 
									{
									/* mov dword ptr Rp[Ir[5]],edi	 */ *Np++ = 0x89; *Np++ = 0xBD; *(LpNUM)Np = (RpRelAddress+(Ir[5]<<BITSIZEOFTVAL)); Np+=4;
									}
								/* mov edi,dword ptr Rp[Am1]	     */ *Np++ = 0x8B; *Np++ = 0xBD; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
								/* mov [edx],edi					 */ *Np++ = 0x89;	*Np++ = 0x3A;
								if (Ir[5] != 0) 
									{
									/* mov edi,dword ptr Rp[Ir[5]]	 */ *Np++ = 0x8B; *Np++ = 0xBD; *(LpNUM)Np = (RpRelAddress+(Ir[5]<<BITSIZEOFTVAL)); Np+=4;
									}
								}
							}
						else
				...
				case vmregSetNumber:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/* mov  edx,Rp[Am2]                  */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						/* fld  qword ptr Rp[Am1]			 */ *Np++ = 0xDD; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
				...
				case vmregSetXInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* add ecx,dword ptr Rp[Am3]         */ *Np++ = 0x03; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL)); Np+=4;
						/* mov edx,dword ptr Rp[Am1]	     */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* mov [ecx],edx					 */ *Np++ = 0x89;	*Np++ = 0x11;
						}
					else
						...
						if ((Rp[modifier[2]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
							...
							/* mov edx,dword ptr Rp[Am2]             */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
							/* shl edx,2                             */ *Np++ = 0xC1; *Np++ = 0xE2; *Np++ = BITSIZEOFNUM; 	
							/* add edx,dword ptr Rp[Am3]             */ *Np++ = 0x03; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL)); Np+=4;
							if (Rp[modifier[0]].Offset >= 1)
								{
								/* mov [edx],Reg[Am1]				 */ *Np++ = 0x89;	*Np++ = REGBEDX[Rp[modifier[0]].Offset];
								}
						...
						if ((Rp[modifier[2]].Offset >= 1) && (Rp[modifier[1]].Offset == 0))
							...
							if (Rp[modifier[0]].Offset >= 1)
								...
								if (Ir[k] != 0) 
									{
									/* mov dword ptr Rp[Ir[k]],Reg[n]*/ *Np++ = 0x89; *Np++ = REGRPMOD[k]; *(LpNUM)Np = (RpRelAddress+(Ir[k]<<BITSIZEOFTVAL)); Np+=4;
									}
								/* mov Reg[n],dword ptr Rp[Am1]	     */ *Np++ = 0x8B; *Np++ = REGRPMOD[k]; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
								/* mov [Am3+(edx*4)],Reg[n]		     */ *Np++ = 0x89; *Np++ = (char)(0x04+REGTARG[k]); *Np++ = (char)(0x80+REGBASE[Rp[modifier[2]].Offset]+REGTARG[6]);
								if (Ir[k] != 0) 
									{
									/* mov Reg[k],dword ptr Rp[Ir[k]]*/ *Np++ = 0x8B; *Np++ = REGRPMOD[k]; *(LpNUM)Np = (RpRelAddress+(Ir[k]<<BITSIZEOFTVAL)); Np+=4;
									}
								}
							}
						else
						if ((Rp[modifier[2]].Offset == 0) && (Rp[modifier[1]].Offset >= 1))
							...
							if (Rp[modifier[0]].Offset >= 1)
								...
								if (Ir[k] != 0) 
									{
									/* mov dword ptr Rp[Ir[k]],Reg[k]*/ *Np++ = 0x89; *Np++ = REGRPMOD[k]; *(LpNUM)Np = (RpRelAddress+(Ir[k]<<BITSIZEOFTVAL)); Np+=4;
									}
								/* mov Reg[k],dword ptr Rp[Am1]	     */ *Np++ = 0x8B; *Np++ = REGRPMOD[k]; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
				...
				case vmregSetXNumber:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* add  edx,Rp[Am3]                  */ *Np++ = 0x03; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));	Np+=4;
						/* fld  qword ptr Rp[Am1]			 */ *Np++ = 0xDD; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* fstp qword ptr [edx]	             */ *Np++ = imod[0]; *Np++ = 0x1A; 
						}
					else
				...
				case vmregSetCharImmediate:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/* mov   edx,dword ptr Rp[pointer]		*/ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						/*** Set Pc/Nc allignment			  ***/ Jp[++n] = (NUM)((Np-ncodes)+2); 
				...
				case vmregSetCharacter:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/* mov   ecx,dword ptr Rp[pointer]		*/ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						/* movsx eax,byte ptr Rp[source]		*/ *Np++ = 0x0F; *Np++ = 0xBE; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						...
						if (Rp[modifier[1]].Offset == 0)
							...
							if (Rp[modifier[0]].Offset >= 4)
								...
								{
								REGSAVEEBX
								/* mov ebx,dword ptr Rp[Am1]	     */ *Np++ = 0x8B; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
								/* mov byte ptr [edx],bl			 */ *Np++ = 0x88;	*Np++ = 0x1A;
								REGLOADEBX
								}
							}
				...
				case vmregSetXCharacter:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/* mov   ecx,Rp[index]              */ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						/* add   ecx,Rp[pointer]            */ *Np++ = 0x03; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));	Np+=4;
						/* movsx edx,byte ptr Rp[source]    */ *Np++ = 0x0F; *Np++ = 0xBE; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						...
						if ((Rp[modifier[2]].Offset >= 1) && (Rp[modifier[1]].Offset >= 1))
							...
							if (Rp[modifier[0]].Offset >= 4)
								...
								{
								REGSAVEEBX
								/* mov ebx,dword ptr Rp[Am1]			*/ *Np++ = 0x8B; *Np++ = REGRPMOD[3]; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
								/* mov byte ptr [edx],bl    			*/ *Np++ = 0x88; *Np++ = (char)(0x02+REGTARG[3]);
								REGLOADEBX
								}
							}
				...
				case vmregSetXShort:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/* mov edx,Rp[index]                */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						/* shl edx,BITSIZEOFSHORT           */ *Np++ = 0xC1; *Np++ = 0xE2; *Np++ = BITSIZEOFSHORT;
						/* add edx,Rp[pointer]              */ *Np++ = 0x03; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));	Np+=4;
						/* movsx eax,word ptr Rp[source]    */ *Np++ = 0x0F; *Np++ = 0xBF; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
				...
				case vmregShlInteger:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/*** No hardware registers		  ***/
						/* mov  cl,byte ptr Rp[source]      */ *Np++ = 0x8A; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* shl  Rp[target],cl               */ *Np++ = 0xD3; *Np++ = imod[0]; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
						{
				...
				case vmregJmpLTNumber:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/* fld    qword ptr Rp[source]			*/ *Np++ = 0xDD; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/* fcomp  qword ptr Rp[comparitor]		*/ *Np++ = 0xDC; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
						{
						...
						if ((Rp[modifier[0]].Offset == 1) && (Rp[modifier[1]].Offset == 0))
							{
							/* fcom   qword ptr Rp[comparitor]  */ *Np++ = 0xDC; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
							}
						else
						if ((Rp[modifier[0]].Offset >= 1) && (Rp[modifier[1]].Offset == 0))
							{
				...
				case vmregShlImmediate:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/*** Set Pc/Nc allignment          ***/ Jp[++n] = (NUM)((Np-ncodes)+6); 
						/* shl dword ptr Rp[Am2],immediate  */ *Np++ = imod[0]; *Np++ = imod[1]; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4; *Np++ = (unsigned char)pcodes[n];
						}
					else
					if (Rp[modifier[1]].Offset == 0)
						{
				...
				case vmregSetXWord:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
							/* mov ecx,dword ptr [ebp+Am3]     */ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
							/* mov edx,dword ptr [ebp+Am2]     */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
							/* shl edx,4                       */ *Np++ = 0xC1; *Np++ = 0xE2; *Np++ = 0x04;
							/* add ecx,edx                     */ *Np++ = 0x03; *Np++ = 0xCA;
							/* mov edx,dword ptr [ebp+Am1]     */ *Np++ = 0x8B; *Np++ = 0x95;*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
							/*** Set Pc/Nc allignment        ***/ Jp[++n] = (NUM)((Np-ncodes)+2); 	
					...
					else
					{
						if (Rp[modifier[2]].Offset == 0)
						{
							/* mov edx,dword ptr [ebp+Am3]     */ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
							if (Rp[modifier[1]].Offset == 0)
							{
								REGSAVEECX
								/* mov ecx,dword ptr [ebp+Am2] */ *Np++ = 0x8B; *Np++ = 0x8D; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
								/* shl ecx,4                   */ *Np++ = 0xC1; *Np++ = 0xE1; *Np++ = 0x04;
							...
							if (Rp[modifier[0]].Offset == 0)
							{
								/* mov ecx,dword ptr [ebp+Am1] */ *Np++ = 0x8B; *Np++ = 0x8D;*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
							}
							else
							{
				...
				case vmregLoadAddress:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
        				/*** source = (Rp[Am1]+offset);   ***/
						/* mov  eax,dword ptr Rp[Am1]       */ *Np++ = 0x8B;*Np++ = 0x85;*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/*** Set Pc/Nc allignment         ***/ Jp[++n] = (NUM)((Np-ncodes)+1); 
						/* add  eax,0x11223344              */ if (pcodes[n]!=0){*Np++ = 0x05;*(LpNUM)Np = pcodes[n];Np+=4;}
						/* mov  dword ptr Rp[Am2],eax       */ *Np++ = 0x89;*Np++ = 0x85;*(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset == 0))
						{
					...
					if ((Rp[modifier[0]].Offset == 0) && (Rp[modifier[1]].Offset >= 1))
						{
						/* mov  Reg[Am2],dword ptr Rp[Am1]  */ *Np++ = 0x8B;*Np++ = REGRPMOD[Rp[modifier[1]].Offset];*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						/*** Set Pc/Nc allignment         ***/ Jp[++n] = (NUM)((Np-ncodes)+2); 
				...
				case vmregMoveImmediate:
					...
					if (Vr[HARDWARESW] == FALSE)
						{
						/*** Set Pc/Nc allignment          ***/ Jp[++n] = (NUM)((Np-ncodes)+6); 
						/* mov Rp[Am2],immediate             */ *Np++ = 0xC7; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); *(LpNUM)Np = pcodes[n]; Np+=4;
						}
					else
						{
						if (Rp[modifier[1]].Offset == 0)
				...
				case vmregLoadNumber:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* mov  dword ptr Rp[Am2],edx       */ *Np++ = 0x89;*Np++ = 0x95;*(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						/* mov  dword ptr (Rp[Am2])+4,ecx   */ *Np++ = 0x89;*Np++ = 0x8D;*(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)+4);Np+=4;
						}
					else
						{
						if (Rp[modifier[0]].Offset == 0)
				...
				case vmregObjLength:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* sub    edx,edx                      */ *Np++ = 0x2B;*Np++ = 0xD2;
						/*** VM___LAST:                      ***/
						/* mov    dword ptr Rp[target],edx     */ *Np++ = 0x89;*Np++ = 0x95;*(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL));Np+=4;
						}
					else
						{
						REGSAVEEAX
				...
				case vmregSaveNumber:
					...
					if (Vr[HARDWARESW] == FALSE)
						...
						/* mov    eax,dword ptr Rp[Am1]        */ *Np++ = 0x8B;*Np++ = 0x85;*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));Np+=4;
						/* mov    dword ptr [edx],eax          */ *Np++ = 0x89;*Np++ = 0x02;
						/* mov    eax,dword ptr (Rp[Am1])+4    */ *Np++ = 0x8B;*Np++ = 0x85;*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)+4);Np+=4;
						/* mov    dword ptr [edx+4],eax	       */ *Np++ = 0x89;*Np++ = 0x42;*Np++ = 0x04;
						/* mov    byte ptr [edx+15],TYREAL     */ *Np++ = 0xC6;*Np++ = 0x42;*Np++ = 0x0F;*Np++ = TYREAL;
						}
					else
				...
				case vmregStringCompare:
					...
					if ((Vr[HARDWARESW] == TRUE) && (Rp[modifier[0]].Offset >= 1))
						...
						{
						/* mov    esi,dword ptr Rp[Am1]    */ *Np++ = 0x8B;*Np++ = 0xB5;*(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL)); Np+=4;
						}
					/*** Set Pc/Nc allignment            ***/ Jp[++n] = (NUM)((Np-ncodes)+2); 
					...
					/*** VM___LAST:                      ***/
					/* mov    dword ptr Rp[target],eax     */ *Np++ = 0x89;*Np++ = 0x85;*(LpNUM)Np = (RpRelAddress+(modifier[2]<<BITSIZEOFTVAL));Np+=4;
				...
				case vmvecInitialize:
					...
					switch ((initEXTENT = pcodes[n]))
						...
						case 0: 
							/*** argument				  ***/
							/* mov   edx,dword ptr Rp[1]	*/ *Np++ = 0x8B; *Np++ = 0x95; *(LpNUM)Np = (RpRelAddress+(pcode.u.Am2<<BITSIZEOFTVAL));Np+=4;
				...
				case vmvecPopNumber:
					...
					/*** Set Pc/Nc allignment          ***/ Jp[n]   = (NUM)(Np-ncodes);        
					/* fstp qword ptr Rp[target]         */ *Np++ = 0xDD; *Np++ = 0x9D; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));	Np+=4;
					break;
				...
				case vmvecPushNumber:
					outStream(JITOp,FALSE)	// FALSE if compiling this instruction instream.
					/*** Set Pc/Nc allignment          ***/ Jp[n]   = (NUM)(Np-ncodes);        
					/* fld  qword ptr Rp[target]         */ *Np++ = 0xDD; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[0]<<BITSIZEOFTVAL));	Np+=4;
					break;
				...
				case  vmvecNumScalar:
					...
					switch (pcodes[n])
						...
						case 0:	/* distance */        
							...
							/* shl   eax,4					*/ *Np++ = 0xC1; *Np++ = 0xE0; *Np++ = 0x04; 
							/* mov   edi,dword ptr [eax+Rp]	*/ *Np++ = 0x8B; *Np++ = 0xBC; *Np++ = 0x28; *(LpNUM)Np = RpRelAddress; Np+=4;
							/* mov   eax,Rp[counter]		*/ *Np++ = 0x8B; *Np++ = 0x85; *(LpNUM)Np = (RpRelAddress+(modifier[1]<<BITSIZEOFTVAL)); Np+=4;
...
switch (pcode.u.Pcode)
        ...
        switch (source->Tag)
            ...
            case TYBOLE:
                switch (argument->Tag)
                    ...
                    case TYCHAR:
                    case TYBOLE:
                        overFlowReg = source->u.Int + argument->u.Int;
						JumpNoOverflow(vmadd001)
                        target->u.Real = (REAL)source->u.Int + (REAL)argument->u.Int;
						target->Tag = TYREAL;
						goto vmadd002;
					vmadd001:
                        target->u.Int = overFlowReg;
						target->Tag = TYNUM;
					vmadd002:
                        break;
            ...
            case TYFLTVECTOR:
				if (argument->Tag == TYFLTVECTOR)
					...
					if (source->u.Object == argument->u.Object) 
						...
						if (ai < ai) {Ip = setIpFromOffset(n); break;}
        ...
        switch (source->Tag)
            ...
            case TYFLTVECTOR:
				if (argument->Tag == TYFLTVECTOR)
					...
					if (si < ai) {Ip = setIpFromOffset(n); break;}
    ...
    case vmregAddImmediate:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) += *(Ip++);
		jitOfflineExit;
		break;
    ...
    case vmregAbsNumber:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = fabs((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))));
		jitOfflineExit;
		break;
    ...
    case vmregInteger:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregNumber:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregAddInteger:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) += (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregAddNumber:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) += (*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregAddPointer:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) += ((*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))<<*(Ip++));
		jitOfflineExit;
		break;
    ...
    case vmregCosNumber:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = cos((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))));
		jitOfflineExit;
		break;
    ...
    case vmregDivrNumber:
		...
		jitOfflineEntry
		source = (LpTVAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL));
		target = (LpTVAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL));
    ...
    case vmregIncPointer:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) += (*(Ip+1) << *(Ip));Ip+=2;
		jitOfflineExit;
		break;
    ...
    case vmregJmpEQImmediate:
		...
		jitOfflineEntry
		n=*(Ip++);if ((*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))) == m) Ip = setIpFromOffset(n);
		jitOfflineExit;
		break;
    ...
    case vmregJmpLTImmediate:
		...
		jitOfflineEntry
		m=*(Ip++);n=*(Ip++);if ((*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))) < m) Ip = setIpFromOffset(n);
		jitOfflineExit;
		break;
    ...
    case vmregJmpEQInteger:
		...
		jitOfflineEntry
		n=*(Ip++);if ((*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))) == (*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))) Ip = setIpFromOffset(n);
		jitOfflineExit;
		break;
    ...
    case vmregJmpEQNumber:
		...
		jitOfflineEntry
		n=*(Ip++);if ((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))) == (*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))) Ip = setIpFromOffset(n);
		jitOfflineExit;
		break;
    ...
    case vmregJump:
		...
		jitOfflineEntry
		Ip = (LpNUM)(*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregLoadAddress:
		...
		jitOfflineEntry
		*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = (NUM)(Rp[pcode.u.Am1].u.Int + *(Ip++));
		jitOfflineExit;
		break;
    ...
    case vmregLoadInteger:
		...
		jitOfflineEntry
		*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = ((TVAL*)(Rp[pcode.u.Am1].u.Int + *(Ip++)))->u.Int;
		jitOfflineExit;
		break;
    ...
    case vmregLoadTail:
		...
		jitOfflineEntry
		*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = asTail(((TVAL*)(Rp[pcode.u.Am1].u.Int + *(Ip++))));
		jitOfflineExit;
		break;
    ...
    case vmregLoadDeclType:
		...
		jitOfflineEntry
		*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = ((TVAL*)(Rp[pcode.u.Am1].u.Int + *(Ip++)))->DeclaredType;
		jitOfflineExit;
		break;
    ...
    case vmregLoadType:
		...
		jitOfflineEntry
		*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = ((TVAL*)(Rp[pcode.u.Am1].u.Int + *(Ip++)))->Tag;
		jitOfflineExit;
		break;
    ...
    case vmregLoadJmpPointer:
		...
		jitOfflineEntry
		*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = (NUM)setIpFromOffset(*(Ip++));
		jitOfflineExit;
		break;
    ...
    case vmregLoadNumber:
		...
		jitOfflineEntry
		*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = ((TVAL*)(Rp[pcode.u.Am1].u.Int + *(Ip++)))->u.Real;
		jitOfflineExit;
		break;
    ...
    case vmregLogNumber:
		...
		jitOfflineEntry
		*((LpREAL)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))) = log(*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))/log(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregObjPointer:
		...
		jitOfflineEntry
        argument	= (TVAL*)(Rp[pcode.u.Am1].u.Int + *(Ip++));
        target      = (TVAL*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL));
    ...
    case vmregPwrNumber:
		...
		jitOfflineEntry
		*((LpREAL)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))) = pow(*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))),*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregRefCharacter:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = *(*((LpCHAR*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregRefFloat:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = *(*((LpFLOAT*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregRefLong:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = *(*((LpNUM32*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
	...
	case vmregRefWord:
		...
		jitOfflineEntry
        (*((LpTVAL)(Rp[pcode.u.Am2].u.Int + *(Ip++)))) = *(*((LpTVAL*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregRefShort:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = *(*((LpSHORT*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregRefXCharacter:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) = (*((LpCHAR*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))];
		jitOfflineExit;
		break;
    ...
    case vmregRefXFloat:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) = (*((LpFLOAT*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))];
		jitOfflineExit;
		break;
    ...
    case vmregRefXInteger:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) = (*((LpNUM*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))];
		jitOfflineExit;
		break;
    ...
    case vmregRefXLong:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) = (*((LpNUM32*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))];
		jitOfflineExit;
		break;
    ...
    case vmregRefXNumber:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) = (*((LpREAL*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))];
		jitOfflineExit;
		break;
    ...
    case vmregRefXShort:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) = (*((LpSHORT*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))];
		jitOfflineExit;
		break;
	...
	case vmregRefXWord:
		...
		jitOfflineEntry
        (*((LpTVAL)(Rp[pcode.u.Am3].u.Int + *(Ip++)))) = (*((LpTVAL*)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))];
		jitOfflineExit;
		break;
    ...
    case vmregSaveInteger:
		...
		jitOfflineEntry
		(target = (TVAL*)(Rp[pcode.u.Am2].u.Int + *(Ip++)))->u.Int = *((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)));
		target->Tag = TYNUM;
		jitOfflineExit;
		break;
    ...
    case vmregSaveTail:
		...
		jitOfflineEntry
		asTail((target = (TVAL*)(Rp[pcode.u.Am2].u.Int + *(Ip++)))) = *((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)));
		jitOfflineExit;
		break;
    ...
    case vmregSaveDeclType:
		...
		jitOfflineEntry
		(target = (TVAL*)(Rp[pcode.u.Am2].u.Int + *(Ip++)))->DeclaredType = (CHAR)*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)));
		jitOfflineExit;
		break;
    ...
    case vmregSaveNumber:
		...
		jitOfflineEntry
		(target = (TVAL*)(Rp[pcode.u.Am2].u.Int + *(Ip++)))->u.Real = *((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)));
		target->Tag = TYREAL;
		jitOfflineExit;
		break;
    ...
    case vmregSetCharImmediate:
		...
		jitOfflineEntry
		*(*((LpCHAR*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (CHAR)(*(Ip++));
		jitOfflineExit;
		break;
    ...
    case vmregSetCharacter:
		...
		jitOfflineEntry
		*(*((LpCHAR*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSetFloat:
		...
		jitOfflineEntry
		*(*((LpFLOAT*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSetIntImmediate:
		...
		jitOfflineEntry
		*(*((LpNUM*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (NUM)(*(Ip++));
		jitOfflineExit;
		break;
    ...
    case vmregSetLongImmediate:
		...
		jitOfflineEntry
		*(*((LpNUM32*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (NUM32)(*(Ip++));
		jitOfflineExit;
		break;
    ...
    case vmregSetLong:
		...
		jitOfflineEntry
		*(*((LpNUM32*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSetShortImmediate:
		...
		jitOfflineEntry
		*(*((LpSHORT*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (SHORT)(*(Ip++));
		jitOfflineExit;
		break;
    ...
    case vmregSetShort:
		...
		jitOfflineEntry
		*(*((LpSHORT*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
	...
	case vmregSetWord:
		...
		jitOfflineEntry
        *(*(LpTVAL*)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))) = *((LpTVAL)(Rp[pcode.u.Am1].u.Int + *(Ip++)));
		jitOfflineExit;
		break;
    ...
    case vmregSetXCharImmediate:
		...
		jitOfflineEntry
		(*((LpCHAR*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (CHAR)(*(Ip++));
		jitOfflineExit;
		break;
    ...
    case vmregSetXCharacter:
		...
		jitOfflineEntry
		(*((LpCHAR*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSetXFloat:
		...
		jitOfflineEntry
		(*((LpFLOAT*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSetXLongImmediate:
		...
		jitOfflineEntry
		(*((LpNUM32*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (NUM32)*(Ip++);
		jitOfflineExit;
		break;
    ...
    case vmregSetXInteger:
		...
		jitOfflineEntry
		(*((LpNUM*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSetXLong:
		...
		jitOfflineEntry
		(*((LpNUM32*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSetXNumber:
		...
		jitOfflineEntry
		(*((LpREAL*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSetXShortImmediate:
		...
		jitOfflineEntry
		(*((LpSHORT*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (SHORT)(*(Ip++));
		jitOfflineExit;
		break;
	...
	case vmregSetXWord:
		...
		jitOfflineEntry
        (*((LpTVAL*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (*((LpTVAL)(Rp[pcode.u.Am1].u.Int + *(Ip++))));
		jitOfflineExit;
		break;
    ...
    case vmregSetXShort:
		...
		jitOfflineEntry
		(*((LpSHORT*)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))))[(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))))] = (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregShlImmediate:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) << *(Ip++);
		jitOfflineExit;
		break;
    ...
    case vmregShlInteger:
		...
		jitOfflineEntry
		(*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = (*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) << (*((LpNUM)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));
		jitOfflineExit;
		break;
    ...
    case vmregSinNumber:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = sin((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))));
		jitOfflineExit;
		break;
    ...
    case vmregSqrtNumber:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = sqrt((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))));
		jitOfflineExit;
		break;
    ...
    case vmregStringCompare:
		...
		srcP = (source->Tag == TYTEXT) ? source->u.Text : CharArray(*source);
		argP = (argument->Tag == TYTEXT) ? argument->u.Text : CharArray(*argument);
		*((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))) = (NUM)strcmp(srcP,argP);
		jitOfflineExit;
		break;
    ...
    case vmregStringiCompare:
		...
		// *((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))) = (NUM)stricmp(srcP,argP);
        for (; tolower(*srcP) == tolower(*argP); srcP++, argP++) //include ctype.h to get macro version.
        {       if (*srcP == '\0')
                {       *((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))) = 0;
                        jitOfflineExit;
                        break;
                }
        }
        *((LpNUM)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL))) = tolower(*srcP) - tolower(*argP);
		jitOfflineExit;
		break;
    ...
    case vmregTanNumber:
		...
		jitOfflineEntry
		(*((LpREAL)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL)))) = tan((*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))));
		jitOfflineExit;
		break;
    ...
    case vmvecInitialize:
		...
		switch ((vecInitializeExtent = *(Ip++)))
			{
			case 0: /* argument */
				argP	= (LpCHAR)(*((LpNUM)(REGP+(argPtrID<<BITSIZEOFTVAL))));
				argInc  = (*((LpNUM)(REGP+(argIncID<<BITSIZEOFTVAL))));
				vecInitializeCount = (*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))));
				vecInitializeLabel = (NUM)Ip;
				break;

			case 1: /* source */
				argP	= (LpCHAR)(*((LpNUM)(REGP+(argPtrID<<BITSIZEOFTVAL))));
				srcP	= (LpCHAR)(*((LpNUM)(REGP+(srcPtrID<<BITSIZEOFTVAL))));
				argInc  = (*((LpNUM)(REGP+(argIncID<<BITSIZEOFTVAL))));
				srcInc  = (*((LpNUM)(REGP+(srcIncID<<BITSIZEOFTVAL))));
				vecInitializeCount = (*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))));
				vecInitializeLabel = (NUM)Ip;
				break;
			...
			case 2: /* target */
				argP	= (LpCHAR)(*((LpNUM)(REGP+(argPtrID<<BITSIZEOFTVAL))));
				srcP	= (LpCHAR)(*((LpNUM)(REGP+(srcPtrID<<BITSIZEOFTVAL))));
				tarP	= (LpCHAR)(*((LpNUM)(REGP+(tarPtrID<<BITSIZEOFTVAL))));
				argInc  = (*((LpNUM)(REGP+(argIncID<<BITSIZEOFTVAL))));
				srcInc  = (*((LpNUM)(REGP+(srcIncID<<BITSIZEOFTVAL))));
				tarInc  = (*((LpNUM)(REGP+(tarIncID<<BITSIZEOFTVAL))));
				vecInitializeCount = (*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))));
				vecInitializeLabel = (NUM)Ip;
				break;
    ...
    case vmvecPopNumber:
		...
		jitOfflineEntry;
		(*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL)))) = Vs[VsTopOfStack--];;		
		if ((VsTopOfStack < 0) || (VsTopOfStack >= MAXVECTORCNT)) goto ErrorIllegalInstruction;
		jitOfflineExit;
		break;
    ...
    case vmvecPushNumber:
		...
		jitOfflineEntry;
		Vs[++VsTopOfStack] = (*((LpREAL)(REGP+(pcode.u.Am1<<BITSIZEOFTVAL))));		
		if ((VsTopOfStack < 0) || (VsTopOfStack >= MAXVECTORCNT)) goto ErrorIllegalInstruction;
		jitOfflineExit;
		break;
    ...
    case vmvecNumScalar:
		...
		switch (*(Ip++))
			...
			case 0: /* distance */
				...
				argInc  = (*((LpNUM)(REGP+(argIncID<<BITSIZEOFTVAL))));
				srcInc  = (*((LpNUM)(REGP+(srcIncID<<BITSIZEOFTVAL))));
				n		= (*((LpNUM)(REGP+(pcode.u.Am2<<BITSIZEOFTVAL))));
				sr      = 0.0;
				for (;n > 0; argP+=argInc,srcP+=srcInc,--n) {am = *(LpREAL)srcP - *(LpREAL)argP; sr += (am * am); };
				(*((LpREAL)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) = sqrt(sr);
				break;
			...
			case 1: /* dot product */
				...
				(*((LpREAL)(REGP+(pcode.u.Am3<<BITSIZEOFTVAL)))) = sr;
				break;
