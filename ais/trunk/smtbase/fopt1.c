...
This is the procedure which is used to generate vminstructions directly for VMSUBU
where it is possible.
...
might generate:
 ...
 0000: subu    aminteg aminteg amfboff  2                1                __T0            
 0004: subu    aminteg amfboff amfboff  3                __T0             __T0            
 0008: return  amfboff amvoid  amvoid   __T0            

From this example we can see how a series of VMSUBU instructions will be generated which 
cascade their result via a temporary variable.

#endif

TVAL FOptimize1_subu(LpXCONTEXT gCP, LpTHREAD gTP, TVector* compilerState, TPair* curPair)
{
NUM                 length;
StartFrame
DeclareTVAL(ret);
...
if (curPair != NIL)
    ...
    if (length == 1)
        ...
        asTag(&tmpPair->itsCdr) = TYPAIR;
        asPair(&tmpPair->itsCdr)->itsCar = curPair->itsCar;
        _LastResult(compilerState) = *ret = FOptimize1_arguments(gCP, gTP, compilerState, tmpPair, VMMULU);
        ExitOnError(*ret);
        }
    else
        {
        ...
        _LastResult(compilerState) = *ret = FOptimize1_arguments(gCP, gTP, compilerState, curPair, VMSUBU);
        ExitOnError(*ret);
        }
...
if( aSymbol == gCP->FCompile_uaddSym)
    {
    *ret = FOptimize1_add(gCP, gTP, compilerState, curPair, VMUADD);
    }
else
...
if( aSymbol == gCP->FCompile_udivSym)
    {
    *ret = FOptimize1_add(gCP, gTP, compilerState, curPair, VMUDIV);
    }
else
...
if( aSymbol == gCP->FCompile_umodSym)
    {
    *ret = FOptimize1_add(gCP, gTP, compilerState, curPair, VMUDIVR);
    }
else
if( aSymbol == gCP->FCompile_umodSym)
    {
...
if( aSymbol == gCP->FCompile_umulSym)
    {
    *ret = FOptimize1_add(gCP, gTP, compilerState, curPair, VMUMUL);
    }
else
...
if( aSymbol == gCP->FCompile_usubSym)
    {
    *ret = FOptimize1_add(gCP, gTP, compilerState, curPair, VMUSUB);
    }
else
...
if( aSymbol == gCP->FCompile_adduSym)
    {
    *ret = FOptimize1_add(gCP, gTP, compilerState, curPair, VMADDU);
    }
else
...
if( aSymbol == gCP->FCompile_divuSym)
    {
    *ret = FOptimize1_div(gCP, gTP, compilerState, curPair, VMDIVU);
    }
else
...
if( aSymbol == gCP->FCompile_muluSym)
    {
    *ret = FOptimize1_mul(gCP, gTP, compilerState, curPair, VMMULU);
    }
else
...
if( aSymbol == gCP->FCompile_subuSym)
    {
    *ret = FOptimize1_subu(gCP, gTP, compilerState, curPair);
    }
else
...
if( aSymbol == gCP->FCompile_divruSym)
    {
    *ret = FOptimize1_twoOp(gCP, gTP, compilerState, curPair, VMDIVRU);
    }
else
...
for a given instruction, it is used for 
     ...
     VMADD VMADDI VMADDU VMDIVR VMSHL VMSHR VMMUL VMMULI VMMULU VMSUB VMSUBI and VMSUBU 
     VMADDN VMDIVN VMMULN VMSUBN 
     VMCADD VMCDIV VMCMUL VMCSUB 
     VMIADD VMIDIVR VMIDIV VMIMUL VMISUB 
     VMUADD VMUDIVR VMUDIV VMUMUL VMUSUB 
...
if (curPair->itsCar.Tag == TYPAIR)
    ...
    if (tmpPair->itsCar.u.Symbol == gCP->FCompile_refmacroSym)
		...
		*args = curPair->itsCdr;
		...
		if (macro->u.Lambda == NULL) 
		   {
		   *ret = TERROR("!attempt to evaluate unknown or void macro!");
		   FrameExit(*ret);
		   }
