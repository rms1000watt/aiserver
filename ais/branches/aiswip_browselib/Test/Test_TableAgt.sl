...
;;
;;  
;;  Title:    tableLambda Test Suite 
;;
;;  Author:   Michael F. Korns, Tim May
;;
;;  Project:  AIS Regression Suite
;;  Notes:    tableLambda data access and assignment study.
;;            The speed and efficiency of relational access
;;            is acceptance tested in this test script.
;;
;;  Notes:    RegTest.sl 
;;
...
;; *****************************************************
;;  Initalize the required tableLambda.
;;******************************************************

;;  Create the required tableLambda.
(tableLambda)
(define ta (new tableLambda))
(ta.setSparse false)
(ta.addColumn Name: Address: Salary:)
(if (<> ta.colCount 3) (testError "column count is wrong!"))
(ta.addRow #{Name: "Michael Korns" Address: "27951 Via Ambrosa" Salary: $100000})
(ta.addRow #{Name: "Jeff Shusterman" Address: "1600 Montebella" Salary: $80000})
(ta.addRow #{Name: "Gilda Cabral" Address: "27951 Via Ambrosa" Salary: $60000})
(if (<> ta.rowCount 3) (testError "row count is wrong!"))
(ta.show 0)

;;  Test sort child Lambda.
(ta.sort (lambda(x y) (< x.Salary y.Salary)))
(if (<> ta[2].Name "Michael Korns") (testError "sort is wrong!"))
(ta.show 0)

;;  Test find child Lambda.
(ta.find (lambda(x) (>= x.Salary $70000)))
(if (<> ta.rowCount 2) (testError "find is wrong!"))
(ta.show 0)

;(setq _path "e:/abase/test/")
(ta.setSparse false)
(gc)
(writeln "Import dense rows into tableLambda.")
(setq startTime (getTickCount 0))
(ta.importTab "Test_TableAgt_stocks.sbf")
(setq endTime (getTickCount startTime))
(writeln "Time to import " ta.rowCount " dense rows into tableLambda was " endTime " seconds")
(if (<> ta.rowCount 488) (testError "dense importTab is wrong!"))

(gc)
(writeln "Export dense rows from tableLambda.")
(setq startTime (getTickCount 0))
(ta.exportTab "temp.sbf")
(setq endTime (getTickCount startTime))
(writeln "Time to export " ta.rowCount " dense rows from tableLambda was " endTime " seconds")
(if (<> ta.rowCount 488) (testError "dense exportTab is wrong!"))

(ta.setSparse true)
(gc)
(writeln "Import sparse rows into tableLambda.")
(setq startTime (getTickCount 0))
(ta.importTab "Test_TableAgt_stocks.sbf")
(setq endTime (getTickCount startTime))
(writeln "Time to import " ta.rowCount " sparse rows into tableLambda was " endTime " seconds")
(if (<> ta.rowCount 488) (testError "sparse importTab is wrong!"))

(gc)
(writeln "Export sparse rows from tableLambda.")
(setq startTime (getTickCount 0))
(ta.exportTab "temp.sbf")
(setq endTime (getTickCount startTime))
(writeln "Time to export " ta.rowCount " sparse rows from tableLambda was " endTime " seconds")
(if (<> ta.rowCount 488) (testError "sparse exportTab is wrong!"))

(testEnd "Test_TableAgt.sl")
