...
void AHttpClient::init(long iDefMsecToWait)
{
	...
	connect(this, SIGNAL(readyRead(const QHttpResponseHeader&)),
			this, SLOT(dataSlot(const QHttpResponseHeader&)));
...
void AHttpClient::launchSetHost()
{
	...
	{	cHost = cNewHost;
		...
		ushort aPort = 80;
		long aPos;
		if ((aPos = aHost.indexOf(':')) > 0)
		{	aHost = cHost.left(aPos);
			aPort = cHost.mid(aPos + 1).toUShort();
			if (aPort <= 0) aPort = 80;
		}
		cPendingTask = ceSettingHost;
		cReqId = setHost(aHost, aPort);
		cpTimer->start(cMsecToWait);
	}
}
...
void AHttpClient::requestHttp(const QString& irUrl, const QString& irBody, const QString& irFileName, long iMsecToWait)
{
	...
	// url = http://localhost:80/default.htm
	long aPos = 0;
	QString aNewHost, aUrl(irUrl);
	cReqId = -1;
	...
	if (aPos > 0)
	{	aUrl = aUrl.mid(aPos);		// url=localhost:80/default.htm
		if ((aPos = aUrl.indexOf('/')) > 0)
		{	aNewHost = aUrl.left(aPos);
			cReqUri = aUrl.mid(aPos);
		}
		else							// url=localhost:80
		{	aNewHost = aUrl;
			cReqUri = "/";
		}
	}
	else if (aUrl.startsWith("/"))
		cReqUri = aUrl;				// url=/default.htm
	else
		cReqUri = '/' + aUrl;			// url=default.htm
