...
TVAL* FSmartbase_InitWORDArray(LpXCONTEXT gCP,LpTHREAD gTP,NUM* stackIndexPtr,const NUM arraySize) 
{
TVAL* stackArrayPtr;
NUM	  stackIndex;
...
gCP->FSmartbase_CnvFromUInt = (LpFUNTVAL)&FSmartbase_CnvFromUInt;
...
gCP->atoui = (LpATOUI)&_strtoui64;
#else
gCP->atoi = (LpATOI)&atol;
gCP->atoui = (LpATOUI)&strtoul;
#endif
...
FObject_Perm(gCP,gTP,(TObject*)(gCP->TSymbol_addu = TSymbol_MakeUnique(gCP,gTP,(LpCHAR)"addu")),TRUE);
...
FObject_Perm(gCP,gTP,(TObject*)(gCP->TSymbol_divu = TSymbol_MakeUnique(gCP,gTP,(LpCHAR)"divu")),TRUE);
...
FObject_Perm(gCP,gTP,(TObject*)(gCP->TSymbol_divru = TSymbol_MakeUnique(gCP,gTP,(LpCHAR)"divru")),TRUE);
...
FObject_Perm(gCP,gTP,(TObject*)(gCP->TSymbol_mulu = TSymbol_MakeUnique(gCP,gTP,(LpCHAR)"mulu")),TRUE);
...
FObject_Perm(gCP,gTP,(TObject*)(gCP->TSymbol_subu = TSymbol_MakeUnique(gCP,gTP,(LpCHAR)"subu")),TRUE);
...
{
	for (i=0;i<(TYMAXVALIDTYPE+1);++i)
	{
	if (strlen(_TObject_TypeName(i)) == 0)
...
#if 0
FSmartbase_CnvFromUInt

Convert from an unsigned integer into a TVAL.

#endif

TVAL FSmartbase_CnvFromUInt(LpXCONTEXT gCP,LpTHREAD gTP,const UNUM inputUInt)
{
TVAL        ret;
