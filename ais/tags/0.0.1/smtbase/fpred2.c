...
switch(argv[0].Tag)
    ...
    case TYBRICK:
    	/*
        LPtr = (char *)FieldArray(argv[0]);
        LSize = Brick(argv[0])->itsMaxItemIndex;

        switch (argv[1].Tag)
            {
            case TYBRICK:
				if (argv[0].u.Object == argv[1].u.Object) FrameExit(rEQUAL);
                RPtr = (char *)FieldArray(argv[1]);
                RSize = Brick(argv[1])->itsMaxItemIndex;
                goto MemoryCompare;
                break;
        ...
        */
        ...
        if ((asTag(&argv[1]) == TYBRICK) && (asObj(&argv[0]) != NIL) && (asObj(&argv[1]) != NIL))
			{
        	if (argv[0].u.Object == argv[1].u.Object) FrameExit(rEQUAL);
