...
/*
 * Extracted from http://www.codeproject.com/KB/shell/balloontipsarticle.aspx By ramshri
 *	NOTE
 *		This class and logic will work if and only if the following key in the registry is set
 *		HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\EnableToolTips\

 *		There needs to be a manifest file in the output location in order for the close button
 *		to work properly.
...
namespace BalloonCS
{
	...
	{
		/// <summary>
		/// Positions using the exact co-ordinates. So if the co-ordinates are outside the screen,
		/// tip won't be shown.
		/// </summary>
		Absolute,

		/// <summary>
		/// Positions using the co-ordinates as a reference. Regardless of the co-ordinates, the tip will 
	...
	public class MessageBalloon : IDisposable
	{
		...
		protected virtual void Dispose(bool disposing)
		{
			...
			{	if(disposing)
				...
				// after the managed resources are disposed, but before the disposed flag is set to
				// true. If thread safety is necessary, it must be implemented by the client.
			}
			disposed = true;
		}
		...
		private void CreateTool()
		{
			...
			// absolute is used tooltip maynot be shown if coords exceed the corners of the screen
			if(m_absPosn)
			{
		...
		/// TRUE : Positions using the exact co-ordinates,if the co-ordinates are outside the screen, tip
		/// wont be shown.
		/// FALSE : Positions using the co-ordinates as a reference. Regardless of the co-ordinates, the 
		/// tip will always be shown on the screen.
		/// </summary>
		...
		/// <summary>
		/// Sets or gets the stem position in the tip. 
		/// TRUE : The stem of the tip is set to center. An attempt is made to show the tip with the stem
		/// centered, if that would make the tip to be hidden partly, stem is not centered.
