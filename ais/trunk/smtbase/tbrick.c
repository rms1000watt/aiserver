...
for (rowIndex = 0; rowIndex < rowCnt; ++rowIndex)
	...
	for (fieldIndex = 0; fieldIndex < fieldCnt; ++fieldIndex)
		...
		switch (bindPtr->Value.DeclaredType)
			...
			case TYUNUM:
				uintArrayPtr = (LpUNUM)(fieldArrayPtr+bindPtr->Value.Offset);
				repeatCnt = bindPtr->Value.Modifier;
				TBrickOnDiskPtr(aHMemory,theOffset)->itsItemArray[cn++] = TYUNUM;
				*((LpNUM)&TBrickOnDiskPtr(aHMemory,theOffset)->itsItemArray[cn]) = repeatCnt;cn+=sizeof(NUM);
				for (repeatIndex = 0; repeatIndex < repeatCnt; ++repeatIndex)
					{
					*((LpUNUM)&TBrickOnDiskPtr(aHMemory,theOffset)->itsItemArray[cn]) = uintArrayPtr[repeatIndex];cn+=sizeof(UNUM);
					}
				break;
		...
		for (rowIndex = 0; rowIndex < rowCnt; ++rowIndex)
			...
			for (fieldIndex = 0; fieldIndex < fieldCnt; ++fieldIndex)
				...
				switch (type)
					...
					case TYUNUM:
						uintArrayPtr = (LpUNUM)fieldArrayPtr;
						for (repeatIndex = 0; repeatIndex < repeatCnt; ++repeatIndex)
							{
							uintArrayPtr[repeatIndex] = *((LpUNUM)&TBrickOnDiskPtr(anHMemory,0)->itsItemArray[cn]);cn+=sizeof(UNUM);
							}
						fieldArrayPtr += (repeatCnt*sizeof(UNUM));
						break;
...
if (self->itsRowCount > 1)
    ...
    if (bindPtr->Value.Modifier > 1 && bindPtr->Value.DeclaredType != TYCHAR)
        ...
        switch (bindPtr->Value.DeclaredType)
			...
			case TYUNUM:
				ret->Tag = TYUNUM;
				ret->u.UInt = *((LpUNUM)fieldArrayPtr);
				break;
...
if (isNumIndex(&index2))
	...
	switch (bindPtr->Value.DeclaredType)
		...
		case TYUNUM:
			if (!inRange(repeatIndex,0,bindPtr->Value.Modifier)) FrameExit(gCP->TObject_ERROR_BADIDXORKEY); 
			ret->Tag = TYUNUM;
			ret->u.UInt = ((LpUNUM)(fieldArrayPtr+bindPtr->Value.Offset))[repeatIndex];
			break;
...
if (isNumIndex(&index2))
	...
	switch (bindPtr->Value.DeclaredType)
		...
		case TYUNUM:
			if (!inRange(repeatIndex,0,bindPtr->Value.Modifier)) FrameExit(gCP->TObject_ERROR_BADIDXORKEY); 
			ret->Tag = TYUNUM;
			ret->u.UInt = ((LpUNUM)(fieldArrayPtr))[repeatIndex];
			break;
...
switch (bindPtr->Value.DeclaredType)
	...
	case TYUNUM:
		...
		*((LpUNUM)(fieldArrayPtr+bindPtr->Value.Offset)) = (UNUM)newValue.u.Int;
		break;
...
if (isNumIndex(&index2))
	...
	switch (bindPtr->Value.DeclaredType)
		...
		case TYUNUM:
			...
			if (!inRange(repeatIndex,0,bindPtr->Value.Modifier)) FrameExit(gCP->TObject_ERROR_BADIDXORKEY); 
			((LpUNUM)(fieldArrayPtr+bindPtr->Value.Offset))[repeatIndex] = (UNUM)newValue.u.Int;
			break;
...
for (fieldIndex = 0; fieldIndex < fieldCount; ++fieldIndex)
	...
	switch (bindPtr->Value.DeclaredType)
		...
		case TYUNUM:
			bindPtr->Value.Offset = recordByteSize;
			recordByteSize += (bindPtr->Value.Modifier*sizeof(UNUM));
			break;
            ...
            for(repeatIndex = 0; repeatIndex < repeatCnt; repeatIndex++)
                ...
                *ec = FConio_sprintn(gCP,gTP,buf,size, *field);
	            ...
	            switch (bindPtr->Value.DeclaredType)
					...
					case TYUNUM:
						asTag(val) = TYUNUM;
                        asUInt(val) = ((LpUNUM)(fieldArrayPtr + bindPtr->Value.Offset))[repeatIndex];
                        *ec = FConio_sprintn(gCP,gTP,buf,size, *val);
                        ExitOnError(*ec);
						break;
