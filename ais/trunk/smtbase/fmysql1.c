...
/* ----------------------------------------------------------------------------
Global MySQL Initialize counter

In a multi-tasking environment, we only wish to initialize the core MySQL engine 
functions once at the start and end of the process - not at the start and end of 
each multi-tasking context.

Therefore we keep a count of how many MySQL initializations/terminations have been requested.
----------------------------------------------------------------------------- */

NUM		FMySQL1_InitializedCounter = 0;	
...
TVAL FMySQL1_Init(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
StartFrame
DeclareTVAL(ret);
EndFrame
...
/* Register the Lisp functionq for MySQL sql */
...
if (gCP->FMySQL1_Enabled)
    {
	// FCC: mysql_thread_init must be called once for every thread.
	// http://dev.mysql.com/doc/refman/5.6/en/mysql-thread-init.html
    //if ((++FMySQL1_InitializedCounter) <= 1) mysql_thread_init();
	...
	gCP->FMySQL1_UseDecimalMoney = TRUE;
...
TVAL FMySQL1_End(LpXCONTEXT gCP,LpTHREAD gTP,NUM argc, TVAL argv[])
{
StartFrame
DeclareTVAL(ret);
EndFrame
...
if (gCP->FMySQL1_Enabled)
    ...
    *ret = FMySQL1_SqlDisconnectAll(gCP, gTP);
	...
	//if ((--FMySQL1_InitializedCounter) <= 1) mysql_thread_end();
	// FCC: mysql_thread_end() must be called before pthread_exit()
	// http://dev.mysql.com/doc/refman/5.6/en/mysql-thread-end.html
	mysql_thread_end();
...
12. (sql useMoney: flag)
- Enables or disables the use of Money for Decimal types.
- Parameters:
    * flag - true or false
...
/* ******************************************************************************/
/* Manage the SQL useMoney command: (sql useMoney: ...) */
/* ******************************************************************************/
if ((argc >= 1) &&
	(argv[0].Tag == TYSYMBOL) && (strcmp(SymbolArray(argv[0]),"useMoney") == 0))
	{
	*ret = FMySQL1_SqlUseMoney(gCP, gTP, argc, argv);
	} /* (sql useMoney:) */
else
...
if (useDefaultEmbedded)
    ...
    *ret = TIntVector_GetIV1(gCP, gTP, TOBJ(gCP->FMySQL1_IntVector), TINT(0));
    ...
    mysqlPtr = mysql_init(NULL);
	mysql_options(mysqlPtr, MYSQL_OPT_RECONNECT, &enableReconnect);
...
/* CLIENT_MULTI_STATEMENTS
   Tell the server that the client may send multiple statements
   in a single string (separated by ??. If this flag is not set,
   multiple-statement execution is disabled.
   http://dev.mysql.com/doc/refman/5.1/en/mysql-real-connect.html
*/
if (mysql_real_connect((MYSQL*)mysqlPtr, hostPtr, userNamePtr, passWordPtr, databasePtr, 0, NULL, CLIENT_MULTI_STATEMENTS) == NULL)
    {
    ...
    for (fieldIdx = 0; fieldIdx < numFields; fieldIdx++)
        ...
        *query = FUtil2_QuickAppend(gCP,gTP,query,temp);
        ...
        switch(bindPtr->Value.DeclaredType)
			...
			case TYUNUM:
#ifdef _M64
                *temp = TSTRING("BIGINT UNSIGNED");
#else
                *temp = TSTRING("INTEGER UNSIGNED");
#endif				
				break;
...
ProcessRows:
	...
	#endif
					mysqlBind[fieldIdx].is_unsigned = 1;
					if (mysqlBind[fieldIdx].buffer == NULL)
						mysqlBind[fieldIdx].buffer = malloc(sizeof(UNUM));

					((LpUNUM)mysqlBind[fieldIdx].buffer)[0] = ((LpUNUM)fieldPtr)[0];
					break;
...
NUM             rowIdx = 0;
NUM             numRows = 0;
BOLE            moreResults = FALSE;
BOLE            useVector = FALSE;

StartFrame
DeclareTVAL(ec);
DeclareTVAL(query);
DeclareTVAL(result);
DeclareTVAL(multiresult);
DeclareTVAL(ret);
DeclareTVAL(type);
DeclareTVAL(repeats);
DeclareTVAL(val);
			...
			if (mysqlResult != NULL)
				...
				*fieldVector = FMake_Vector(gCP, gTP, 0, NULL);
					...
					switch(mysqlField[fieldIdx].type)
						...
						case MYSQL_TYPE_NEWDECIMAL:
                    		...
                    		if (IS_NOT_NULL(mysqlField[fieldIdx].flags) || !gCP->FMySQL1_UseWordFlt)
								*type = (gCP->FMySQL1_UseDecimalMoney == TRUE) ? TSYMBOL("Money") : TSYMBOL("Number");
                    		else
                    			*type = TSYMBOL("Word");
							break;
						...
						case MYSQL_TYPE_LONG:
							/* Type: Long or Integer */
                    		if (IS_NOT_NULL(mysqlField[fieldIdx].flags) || !gCP->FMySQL1_UseWordInt)
                    			*type = (mysqlField[fieldIdx].flags & UNSIGNED_FLAG) ? TSYMBOL("Integer") : TSYMBOL("Long");
                    		else
                    			*type = TSYMBOL("Word");
							break;

						case MYSQL_TYPE_LONGLONG:
							/* Type: Integer or UnsignedInteger */
							if (IS_NOT_NULL(mysqlField[fieldIdx].flags) || !gCP->FMySQL1_UseWordInt)
								*type = (mysqlField[fieldIdx].flags & UNSIGNED_FLAG) ? TSYMBOL("UInteger") : TSYMBOL("Integer");
                    		else
                    			*type = TSYMBOL("Word");
							break;
							break;
						...
						case MYSQL_TYPE_SHORT:
							/* Type: Object: Short or Long */
                    		if (IS_NOT_NULL(mysqlField[fieldIdx].flags) || !gCP->FMySQL1_UseWordInt)
								*type = (mysqlField[fieldIdx].flags & UNSIGNED_FLAG) ? TSYMBOL("Long") : TSYMBOL("Short");
                    		else
                    			*type = TSYMBOL("Word");
							break;
				...
				if (numRows > 0)
					...
					*result = TBrick_MakeNew(gCP, gTP, 2, &recprm[0]);
					}
				else
					{
            		recprm[0] = *fieldVector;
            		/* Create our Brick structure */
            		*result = TBrick_MakeNew(gCP, gTP, 1, &recprm[0]);
					}

				MySQLExitOnError(*result);
						...
						if (valuePtr != NULL)
							...
							switch(mysqlField[fieldIdx].type)
								...
								case MYSQL_TYPE_BIT:
									...
									*ec =  TBrick_SetIV3(gCP, gTP, *result, TINT(fieldIdx), TINT(0), TINT(rowIdx), *val);
									MySQLExitOnError(*ec);
									break;
								...
								case MYSQL_TYPE_NEWDECIMAL:
									/* Type: Money */
									asTag(val) = (gCP->FMySQL1_UseDecimalMoney == TRUE) ? TYMONEY : TYREAL;
								...
								case MYSQL_TYPE_LONGLONG:
									if (mysqlField[fieldIdx].flags & UNSIGNED_FLAG)
									{
										/* Type: UnsignedInteger */
										*val = TUINT(gCP->atoui(mysqlRow[fieldIdx], NULL, 10));
									}
									else
									{
										/* Type: Integer */
										*val = TINT(gCP->atoi(mysqlRow[fieldIdx]));
									}
								...
								case MYSQL_TYPE_DATE:
									...
									*ec =  TBrick_SetIV3(gCP, gTP, *result,
								...
								case MYSQL_TYPE_VAR_STRING:
									...
									if (mysqlField[fieldIdx].flags & BINARY_FLAG)
										...
										*val = TSTRING(mysqlRow[fieldIdx]);
										...
										/* stored as character */
										*ec = TBrick_SetIV3(gCP, gTP, *result, TINT(fieldIdx), TINT(-1), TINT(rowIdx), *val);
				...
				if (mysql_field_count(mysqlPtr) == 0)
					...
					*result = TINT(mysql_affected_rows(mysqlPtr));
					}
				else
					{
					/* query was SELECT */
					...
					*result = TError_sprintf(gCP, gTP, gTP->TempBuffer, "!sql.query: %s!", errorPtr);
					}
				} /* (mysqlResult != NULL) */

			/* at this point, the result set should have been deallocated */
			moreResults = FALSE;

			if (mysql_more_results(mysqlPtr))
				{
				/* multiple results */
				moreResults = TRUE;

				/* create vector on first encounter */
				if (!useVector)
					{
					*multiresult = FMake_Vector(gCP, gTP, 0, NULL);
					ExitOnError(*multiresult);
					useVector = TRUE;
					}
				/* get next result */
				mysql_next_result(mysqlPtr);
				} /* more results? */

			/* check if we are returning a vector */
			if (useVector)
				{
				/* add result to vector */
				*ec = TVector_AddNewValue(gCP, gTP, *multiresult, *result);
				ExitOnError(*ec);
				}
			} /* do */
		while (moreResults); /* continue? */
...
This procedure enables/disables the use of Money for Decimal types.
(sql useMoney: true)
(sql useMoney: false)

Result: true
#endif
TVAL FMySQL1_SqlUseMoney(LpXCONTEXT gCP, LpTHREAD gTP, NUM argc, TVAL argv[])
{
StartFrame
DeclareTVAL(ret);
EndFrame
...
gCP->FMySQL1_UseDecimalMoney = argv[1].u.Bool;
