...
ASessionManager::~ASessionManager()
{
    ...
    {
        cSessionMgrMutex.lock();
		// Make sure the context is not already in the process of being deleted 
		if (((apContext = cOpenContexts[aContext]) != NULL) &&
			 (apContext->mType == CONTEXTTYPECODE) &&
			 (apContext->mAlive >= 0))
        {
			// the context thread will catch this on the next cycle
...
//	getSession returns the Session object for the specified sessionID.
//	Args:	iSessionId		The session Id of the session to be returned
//  Return:	sessionPtr		The session pointer or NULL if there was an error
ASession* ASessionManager::getSession(long iSessionId)
{
	ASession* apSession;
...
//	getResult returns the last result of an evaluation in the specified session
//	Args:	iSessionId		The session Id of the result to be returned
//  Return:	result			The last result of an evaluation in the specified session
QString ASessionManager::getResult(long iSessionId)
{
	QString		aResult;
	ASession*	apSession;
	...
	{	
		...
		if (apSession->mpContext == NULL)
			goto Error;
		aResult = apSession->mReturnString;
	}
...
//	setResult sets the current result of an evaluation in the specified session
//	Args:	iSessionId		The session Id of the result to be returned
//          irBf			The current result string to set
//  Return:	result			The last result of an evaluation in the specified session
QString ASessionManager::setResult(long iSessionId, QString &irBf)
{
	QString		aResult;
	ASession*	apSession;
...
//	getDisplay returns and resets the current display buffer contents of the specified session
//	Args:	iSessionId		The session Id of the disp[lay buffer to be returned
//  Return:	result			The current display buffer contents in the specified session
QString ASessionManager::getDisplay(long iSessionId)
{
	QString		aResult;
	ASession*	apSession;
...
long ASessionManager::cbCancelRequestHttp(long iSessionId)
{
	AisMgrOutputEvent* apEv = new AisMgrOutputEvent(iSessionId, -1/*ReqId*/, 0/*Status*/, cMt/*Enctype*/, 0/*RetValue*/,
	cMt/*AisOut*/, NULL/*Data*/, 0/*DataSize*/, cMt/*Display*/,cMt/*Error*/, AISMGROUT_CANCELREQUESTHTTP);
...
long ASessionManager::cbRequestHttp(long iSessionId, QString& irUrl, QString& irFileName, QString& irBody, long iMsecToWait)
{
	AisMgrOutputEvent* apEv = new AisMgrOutputEvent(iSessionId, 0/*ReqID*/, 0/*Status*/, irFileName/*Enctype*/, iMsecToWait,
	irUrl/*AisOut*/, NULL/*Data*/, 0/*DataSize*/, irBody/*Display*/, cMt/*Error*/, AISMGROUT_REQUESTHTTP);
	QCoreApplication::postEvent(this, apEv);
	return 0;
}
...
long ASessionManager::cbSendStatusUpdateToClient(long iSessionId, QString& irMsg)
{
	if (cpAisMgr)
	{
		AisMgrOutputEvent* apEv = new AisMgrOutputEvent(iSessionId, -1/*ReqID*/, 0/*Status*/, cMt/*Enctype*/, 0/*RetValue*/
		, irMsg/*AisOut*/, NULL/*Data*/, 0/*DataSize*/, cMt/*Display*/,cMt/*Error*/, AISMGROUT_SENDSTATUSTOCLIENT);
		QCoreApplication::postEvent(cpAisMgr, apEv);
	}
	return 0;
}
...
{
	long			aContextId = -1;
	long			aReqId = AUNKNOWN_CONTEXTNAME;
	AContextSt* apContext;
	long			aContext;
	long			aMaxTries = 1000;			// 30720 MaxTries * 2 Msec = about 1 minute
	long			aTry;
	AContextThread* apThread;
	long			aReq;
	long			aReqSz;
	...
	// Look up context name in context map, and that provides the contextId, which provides the context pointer.
	...
	{	// Set Default CloseMode. Set mode if not already set
		...
		// Remove this context from the cOpenContexts list and from the cContextMap
		// Note: The context thread will catch this on the next cycle
		apContext->mAlive = -1;		// (empty <0, registered =0, opened >0)
		if ((apThread = apContext->mpThread) != NULL)
		{	// Wait for thread to finish current request, if any.
			for (aTry = 0; aTry < aMaxTries; ++aTry)
			...
			apThread->wait();
			cSessionMgrMutex.lock();
			// One or more of these cause an immediate crash ???
			// Note: Without these the crash occurs on shudown
		    // apContext->mpThread = NULL;
		    // delete apThread;
		}
		...
		for (aReq = 0; aReq < aReqSz; ++aReq)
			delete apContext->mRequests[aReq];
		apContext->mContextName = ""; // We will reset the context name so it can be reused by the registerContext method.
		apContext->mAlive = -1;		  // We will reset the context alive switch so it can be reused by the registerContext method.

		// Note: (MFK&FCC) For large grain multi-tasking we need to completely remove each context
		//       when they are closed. Current design allows for contexts to be open by not initialized.
		//       This may benefit from some design discussion.
		cContextMap.remove(irContextName);
	}
...
bool ASessionManager::event(QEvent* ipEv)
{
	...
	{	apHttpClient = new AHttpClient(this, aSessionId, 0, NULL, "ASessionManager");
	...
	switch (aFcnType)
	{
	case AISMGROUT_CANCELREQUESTHTTP:
		if (apHttpClient != NULL)
	...
	{	cSessionMgrMutex.lock();
		...
		{	apReq->mExecuting = true;
			...
			default:
				...
				cSessionMgrMutex.unlock();
                SBGlue_Eval(apContext->mpCP,iSessionId,aRequestID,aUserID,aSecLvl,aReqType,1,0,apContext->mJitMode,apStr,NULL);
...
/*-----------------------------------------------------------------
setSessionEscapeFlag

Summary:	Set the session escape flag which tells the session to stop all execution.

Args:		sessionId		The ID of the session whose escape flag is to be set
Return:		returnCode		The old value of the session escape flag.

Note:		Multitasking session escape request setting/testing.
			The legacy escape testing tries to stop the context.
			In multitasking, we need to stop only the specified session.
			We will need some design review here before proceeding.
------------------------------------------------------------------*/
bool ASessionManager::setSessionEscapeFlag(long iSessionId, bool flag)
{
	ASession* apSession = 0;
	bool      aOldFlag = false;
	...
	if (apSession != NULL)
	{
		aOldFlag = apSession->mEngineEscapeSW;
		apSession->mEngineEscapeSW = flag;
	}
	cSessionMgrMutex.unlock();
...
long ASessionManager::flushSessions(bool iNow)
{
	...
	while (aIter != cSessions.end())
	{
		if (aIter.key() != 0)
		{
			...
			if (apSession != NULL && apSession->mpContext != NULL && !apSession->mDsplyBfr.isEmpty())
...
/*!
\brief getContextSt - Get pointer to context structure

\param irContextName - Name of the context
\return aContextSt (NULL if context is not found)
*/
AContextSt* ASessionManager::getContextSt(const QString& irContextName)
{
	long aContextId;
...
\return aCode - 0 if no error, else error code (<0)
        Note: Returns context sessionID or a negative error code.
 */
		...
		else
		{
			...
			else
			{
				for (aSessionId = 1; aSessionId < cSessions.size(); ++aSessionId)
				{
					...
					if (!cSessions.contains(aSessionId) || cSessions[aSessionId] == NULL || cSessions[aSessionId]->mpContext == NULL)
					{
						break;
					}
				}
			...
			apSession->mEngineFlags = 0;
			apSession->mEngineEscapeSW = false;
			apSession->mpHttpClient = NULL;
			apSession->mMode = geOpen;			// Active and connected
			apSession->mNoDebug = 1;
			apSession->mReadHtmlOk = false;
...
long ASessionManager::requestHttp(long iSessionId,char* ipUrl,char* ipBody,char* ipFileName,const char **opPage,long iMsecToWait)
{
	...
	{	ASession* apSession = cSessions[iSessionId];
		...
		cbRequestHttp(iSessionId, aUrl, aFileName, aBody, iMsecToWait);
		...
		{	cSessionMgrMutex.lock();
			...
			{	aCanceled = true; // cancelRequestHttp just once
				cbCancelRequestHttp(iSessionId);
			}
			apContext->mpThread->localSleep(1L/*Msec*/);
		}
		*opPage = (const char *)(cSessions[iSessionId]->mPageContent);
...
bool ASessionManager::saveConsoleLog(const QString& irDisplay, ARequestID iRqId, ASessionID iSessionId, char iType)
{
	...
	{	cSessionMgrMutex.lock();
		ASession* apSession = (cSessions.contains(iSessionId)) ? cSessions[iSessionId] : NULL;
