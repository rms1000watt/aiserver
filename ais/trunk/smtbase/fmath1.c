...
if (argv[(index = 0)].Tag == TYCPX)
{
	*ret = FConvert_ToComplex(gCP, gTP,argv[index]);
	ExitOnError(*ret);
}
else
{
...
while (++index < argc)
{	
	if (ret->Tag == TYUNUM && argv[index].Tag == TYUNUM)
	{	
		ret->u.UInt -= tmp->u.UInt;
	}
	else
	if (ret->Tag == TYCPX || argv[index].Tag == TYCPX)
	{	if (ret->Tag != TYCPX)	
	...
	if (argv[index].Tag != TYREAL)
	{	
		if (ret->Tag != TYREAL) *ret = FObject_RealAnyCnv(gCP,gTP,TYREAL,*ret);
		*tmp = FObject_RealAnyCnv(gCP,gTP,TYREAL, argv[index]);
		ExitOnError(*tmp);
...
// When we enter for the first time ret holds a UInteger.
while (++index < argc)
{	
	if (ret->Tag == TYUNUM && argv[index].Tag == TYUNUM)
	{	
...
if (argv[0].Tag == TYREAL)
	...
	x = (REAL)argv[0].u.Int;
else if (argv[0].Tag == TYUNUM)
	x = (REAL)argv[0].u.UInt;
else
{	*tmp = FObject_RealAnyCnv(gCP,gTP,TYREAL, argv[0]);
	ExitOnError( *tmp);
	x = tmp->u.Real;
...
if (argv[1].Tag == TYREAL)
    ...
    y = (REAL)argv[1].u.Int;
else if (argv[1].Tag == TYUNUM)
    y = (REAL)argv[1].u.UInt;
else
{	*tmp = FObject_RealAnyCnv(gCP,gTP,TYREAL, argv[1]);
	ExitOnError(*tmp);
	y = tmp->u.Real;
