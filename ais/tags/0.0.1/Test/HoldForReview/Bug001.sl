...
;;           cabinet.
;; Args:     name:    (optional) Name of the script to be displayed.
;;                    If not present, the select function allows
;;                    the user to choose a script from the cabinet.
;; Globals:  _BrowseCabinet  Set to the current file cabinet (main or alternate).
;;           _FileCabinet    Ref as an Lambda file cabinet repository (mandatory). 
;;           _AltFCabinet    Ref as an alternate Lambda file cabinet repository (optional). 
;; Return:   length   The length of the Lambda cabinet source.
;; *******************************************************************
    vars:(name s i n commands keys)
    pvars:(;; Persistent Variables
           (bootRecordKey "**boot-record**") ;; Boot record key (binary data, not source)
           ...
           compileIt               ;; Compile a source string and trap errors
           ...
           eraseBootRecord         ;; Erase the boot record from the file cabinet
           ...
           importSource            ;; Manage source importing 
           inspect                 ;; Return a Structure of symbols and their closure size
           loadBootRecord          ;; Load the boot record from the file cabinet
           readSourceFile          ;; Read source file 
           removeDelimiters        ;; Remove all delimiters from a string
           saveBootRecord          ;; Save the boot record into the file cabinet
           sysErrorStop            ;; Recover from all system errors.
    ...
    ;; confusion when the reinitialized clone begins to affect
    ;; the persistant storage of the original Lambda.
    ;;(if (<> errorHandler #void) (goto Continue:))
    ;; Check an Lambda script into the File Cabinet.
    (defun checkin(LambdaName LambdaScript)
        (eraseBootRecord)
        (setq _BrowseCabinet[LambdaName] LambdaScript)
        true)
    ;; Check an Lambda script out of the File Cabinet.
    (defun checkout(LambdaName) 
       vars:(result)
       (setq result _BrowseCabinet[LambdaName])
       (if (<> result #void) (return result))
       (if (<> _BrowseCabinet _FileCabinet)
           (setq result _FileCabinet[LambdaName])
           (setq result _AltFCabinet[LambdaName])
           ) ; end if
    ...
    (defun compileAll()
        ...
        (if (<> (type _BrowseCabinet) ObjectRepository:) (return false))
        (beginTransaction _BrowseCabinet)
        (loop for _ix from 0 until (length _BrowseCabinet) do
	      (setq _compileLambda _BrowseCabinet[_ix 0])
	      (setq _sourceCode _BrowseCabinet[_ix 1])
            (if (and (<> _compileLambda bootRecordKey)
                     (<> (left _sourceCode 7) ";#text#"))
                (errorTrap  '(eval _sourceCode)  
                            '(writeln "I got a compilation error on " _BrowseCabinet[_ix 0])
                            )) ;; end of errorTrap
            ) ; end loop
        (setq _sourceCode #void)
        (abortTransaction _BrowseCabinet)
        (setq browseLib holdMe)
        true) ;; end of compileAll
    ;; Compiles, and evaluates the specified source string.
    (defun compileIt(aString)
        ;; Capture and return any compile errors as strings
        (onError (lambda(err) err))
        ;; Compile the source string
        (if (<> (left aString 7) ";#text#") (eval aString))
        true)
    ;; Compile the specified Lambda script.
    (defun compileSource(...)
    ;; *******************************************************************
    ;; summary:  Loads, compiles, and evaluates the specified (name) Lambda 
    ;;	     script in the Lambda file cabinet. If no name is specified,
    ;;	     the entire Lambda file cabinet is scanned sequentially.
    ;;	     Each Lambda script is loaded, compiled, and evaluated. If
    ;;	     an error occurs, the Lambda script and the error are
    ;;	     displayed. 
    ;; Args:     name	(optional) Name of the Lambda to be compiled from
    ;;			the Lambda file cabinet. If not present, the 
    ;;			entire Lambda file cabinet is scanned sequentially.
    ;; Return:   false	Always returns false or an error.
    ;; *******************************************************************
        vars:(name keys childName s n startIndex 
              file brk fileID (typ 0) err
              ) ;; end of temporary variables
        ;; Obtain the name of the Lambda to compile.
        ;; We also compile any child Lambdas (if necessary).
        (if (>= (argCount) 1)
	    (begin
               (setq name (argFetch 0))
               ;; If the name is #void, let the user select a name.
               (if (= name #void) (goto CompileAllSource:))
               ;; Do not try to compile the boot record (it is binary, not source).
               (if (= name bootRecordKey) (return false))
               ...
               (setq n (find ":" name))
               (if (isNumber n)
                   (setq name (left name n)))
               ;; Search for any child Lambdas
               (setq childName (append "" name ":"))
               (setq n (length childName))
               (setq startIndex _BrowseCabinet[position: name])
               (if (isNumber startIndex)
                   (begin
                       (beginTransaction _BrowseCabinet)
                       (setq _ix startIndex)
	                 ;(writeln "Starting compile for " _BrowseCabinet[_ix 0])
                       (setq err (compileIt _BrowseCabinet[_ix 1]))
                       (if (isString err)  
                           (writeln (append "Compiling " _BrowseCabinet[_ix 0] ", I got: " err)))
                       (setq _ix (addi _ix 1))
                       (while (and (< _ix (length _BrowseCabinet))
                                   (= (left _BrowseCabinet[_ix 0] n) childName))
	                     ;(writeln "Starting compile for " _BrowseCabinet[_ix 0])
                           (setq err (compileIt _BrowseCabinet[_ix 1]))
                           (if (isString err)  
                               (writeln (append "Compiling " _BrowseCabinet[_ix 0] ", I got: " err)))
               ...
               (return true))           
            (setq name #void))
        ;; Compile each Lambda script in the file cabinet.
        CompileAllSource::
        (compileAll)) ;; end of compileSource
    ...
    ;; Erase the boot record from the file cabinet
    (defun eraseBootRecord() (setq _BrowseCabinet[bootRecordKey] #void) true)
    ;; Erases a source file from the file cabinet. 
    (defun eraseSource(name)
        vars:(s n keys)
        ;; Erase source from Lambda file cabinet.
        (setq s _BrowseCabinet[name])
        (setq _BrowseCabinet[name] #void)
        (if (= s #void)
            (setq s "...no source available..."))
        ;; Return the erased source.
        s)
    ...
    (defun exportSource(fileName LambdaName)
        vars:(name Lambda s n keys
              startIndex endIndex
              parentName parentLen
              childName childLen oldPath
              file brk fileID (typ 0)
              ) ;; end of temporary variables
        ;; Check for a request to export "all" Lambdas.
        (if (= LambdaName "..all Lambdas..") (setq LambdaName ""))
        ;; Do not try to export the boot record (it is binary, not source).
        (if (= LambdaName bootRecordKey) (return false))
        ...
        (if (= LambdaName "")
            ...
            (begin
               (setq startIndex 0)
               (setq endIndex (length _BrowseCabinet))
               ) ;; end of then
            ;; We are exporting one parent and its child Lambdas. 
            (begin
               (setq startIndex _BrowseCabinet[position: parentName])
               (if (isBoolean startIndex) false)
               (setq endIndex (length _BrowseCabinet))
               ) ;; end of else
            ) ;; end of if
        ...
        (fwriteln fileID ";;*************************************")
        (fwriteln fileID ";;*************************************")
        (fwriteln fileID ";; Exported Lambda File Cabinet Document")
        (fwriteln fileID ";;*************************************")
        (fwriteln fileID ";;*************************************")
        ...
        (loop for i from startIndex until endIndex do
            ...
            (setq name _BrowseCabinet[i 0])
            (if (and (<> parentName name)
                     (<> childName (left name childLen)))
                (goto CloseFileCabinet:)
                ) ;; end if
            ;; Export this Lambda (if it is not the boot record).
            (if (<> name bootRecordKey)
                (begin
                   (fwriteln fileID _eol ";;**EXPORTKEY**:" name)
                   (fwriteln fileID _BrowseCabinet[i 1])
                   )) ;; end if
            ) ; end loop
        CloseFileCabinet::
        (abortTransaction _BrowseCabinet) 
    ...
    (defun getKeys()
        vars:(i v)
        (beginTransaction _BrowseCabinet)
        (setq v (new Vector: (length _BrowseCabinet)))
        (loop for i from 0 until (length _BrowseCabinet) do
            (if (<> _BrowseCabinet[i 0] bootRecordKey)
                (setq v[i] _BrowseCabinet[i 0])
                (setq v[i] "...binary boot record..."))
            ) ;; end of loop
        (abortTransaction _BrowseCabinet)
        v)
    ;; Return a vector of all source file names in the file cabinet.
    ...
    (defun importSource(name)
        ...
        (loop for i from 1 until (length v) do
            (setq n (find _eol v[i]))
            (setq key (left v[i] n))
            (writeln "[" i "] Importing source for " key)
            (setq _BrowseCabinet[key] (mid v[i] (+ n (length _eol)) 10000000))
            ) ; end loop
        (writeln "Completed importing [" (-- i) "] Lambdas.")
        (commitTransaction _BrowseCabinet) 
        true)
    ...
    (defun inspect(typeSW)
        ...
        (sort result > byValue:)
        result)
    ;; Load the boot record from the file cabinet
    (defun loadBootRecord()
       vars:(bootRecord i bootRecordLen)
       (setq bootRecord _BrowseCabinet[bootRecordKey])
       (if (= bootRecord #void) (return (compileAll)))
       (setq bootRecordLen (length bootRecord))
       (loop for i from 0 until bootRecordLen do
          (set bootRecord[i 0] bootRecord[i 1])
          ) ;; end loop
       true) ;; end loadBootRecord
    ...
    (defun removeDelimiters(anObj) 
        ...
        result)
    ;; Save the boot record into the file cabinet
    (defun saveBootRecord()
       vars:(nameVec valueVec i n bootRecord)
       (setq nameVec (getParentNames))
       (setq valueVec (new Vector: 0))
       (setq n (length nameVec))
       (loop for i from 0 until n do
          (setq nameVec[i] (symbol nameVec[i]))
          (setq valueVec[i] (getGlobalValue nameVec[i]))
          ) ;; end loop
       (setq bootRecord (objectToStructure nameVec valueVec))
       (setq _BrowseCabinet[bootRecordKey] bootRecord)
       true) ;; end saveBootRecord
    ;; Recover from any system errors which occur.
    (defun sysErrorStop(errCode) (append dString " !sysErr#" errCode))
    ...
    (defun writeSourceFile(name data) 
        ...
        (fileClose fileID 1)
        self)
    ;; Make sure the file cabinet is properly assigned.
    Continue::
    (onError errorHandler)
    ;; Make sure the file cabinet is properly assigned.
    (if (<> (type _FileCabinet) ObjectRepository:)
            (error "browseLib" "The _FileCabinet is not assigned properly."))
    ;; Load and compile all Lambda scripts in the File Cabinet.
    (setq _BrowseCabinet _FileCabinet)
    (loadBootRecord)
    ;; Load and compile all Lambda scripts in the Alternate File Cabinet.
    (setq _BrowseCabinet _AltFCabinet)
    (loadBootRecord)
    ;; Return with the File Cabinet assigned properly.
    (setq _BrowseCabinet _FileCabinet)
    _BrowseCabinet)
    ...
    (if (= passCount 1)
        ...
        (begin
           ...
           (loop for blockIndex from 0 until passCount do
              (setq blockLists[blockIndex] (argFetch (add1 blockIndex)))
